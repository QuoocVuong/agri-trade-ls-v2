...
import org.springframework.mail.MailSendException; // Import MailSendException
import org.springframework.mail.javamail.JavaMailSender;
// import org.springframework.mail.javamail.MimeMessageHelper; // Không cần trực tiếp trong test
...
// import java.util.Properties; // Không cần
...
class EmailServiceImplTest {
    ...
    private void verifyEmailSent(String expectedTemplateName, String expectedRecipient, String expectedSubjectContains) throws MessagingException {
        ArgumentCaptor<Context> contextCaptor = ArgumentCaptor.forClass(Context.class);
        // Sử dụng lenient() cho verify nếu template engine có thể không được gọi trong một số luồng (ví dụ: recipient rỗng)
        ...
        assertTrue(Arrays.stream(sentMessage.getRecipients(MimeMessage.RecipientType.TO)).anyMatch(addr -> addr.toString().equals(expectedRecipient)),
                "Expected recipient " + expectedRecipient + " not found in " + Arrays.toString(sentMessage.getRecipients(MimeMessage.RecipientType.TO)));
        assertTrue(sentMessage.getSubject().contains(expectedSubjectContains),
                "Expected subject to contain '" + expectedSubjectContains + "', but was '" + sentMessage.getSubject() + "'");
    }
    ...
    class UserRelatedEmailTests {
        ...
        void sendVerificationEmail_shouldProcessTemplateAndSend() throws MessagingException {
            ...
            when(thymeleafTemplateEngine.process(eq("mail/email-verification"), any(Context.class))).thenReturn("<html>Verification HTML</html>");
            ...
            ArgumentCaptor<Context> contextCaptor = ArgumentCaptor.forClass(Context.class);
            // verify(thymeleafTemplateEngine).process(anyString(), contextCaptor.capture()); // Dòng này thừa nếu đã verify ở trên
            verify(thymeleafTemplateEngine).process(eq("mail/email-verification"), contextCaptor.capture());
        ...
        void sendWelcomeEmail_shouldProcessTemplateAndSend() throws MessagingException {
            when(thymeleafTemplateEngine.process(eq("mail/welcome"), any(Context.class))).thenReturn("<html>Welcome HTML</html>");
    ...
    class OrderRelatedEmailTests {
        ...
        void sendOrderConfirmationEmailToBuyer_shouldProcessAndSend() throws MessagingException {
            when(thymeleafTemplateEngine.process(eq("mail/order-confirmation-buyer"), any(Context.class))).thenReturn("<html>Order Confirmation</html>");
            emailService.sendOrderConfirmationEmailToBuyer(testOrder); // testOrder.getBuyer() là testBuyer
            verifyEmailSent("mail/order-confirmation-buyer", testBuyer.getEmail(), "Xác nhận đơn hàng #" + testOrder.getOrderCode());
        }

        @Test
        @DisplayName("Send Order Status Update Email To Buyer - Status Changed")
        void sendOrderStatusUpdateEmailToBuyer_whenStatusChanged_shouldSendEmail() throws MessagingException {
            testOrder.setStatus(OrderStatus.SHIPPING);
            OrderStatus previousStatus = OrderStatus.PROCESSING;
            when(thymeleafTemplateEngine.process(eq("mail/order-status-update-buyer"), any(Context.class))).thenReturn("<html>Status Update</html>");

            emailService.sendOrderStatusUpdateEmailToBuyer(testOrder, previousStatus);

            verifyEmailSent("mail/order-status-update-buyer", testBuyer.getEmail(), "Cập nhật trạng thái đơn hàng #" + testOrder.getOrderCode());
        }
    ...
    class ProductApprovalEmailTests {
        ...
        void sendProductApprovedEmailToFarmer_success() throws MessagingException {
            when(thymeleafTemplateEngine.process(eq("mail/product-approved-farmer"), any(Context.class))).thenReturn("<html>Approved</html>");
            emailService.sendProductApprovedEmailToFarmer(testProduct, testFarmer);
            verifyEmailSent("mail/product-approved-farmer", testFarmer.getEmail(), "Sản phẩm của bạn đã được duyệt");
        }
    ...
    class InvoiceEmailTests {
        ...
        void sendOverdueInvoiceReminderEmail_success() throws MessagingException {
            when(thymeleafTemplateEngine.process(eq("mail/invoice-overdue-reminder"), any(Context.class))).thenReturn("<html>Overdue</html>");
            emailService.sendOverdueInvoiceReminderEmail(testInvoice); // testInvoice.getOrder().getBuyer() là testBuyer
            verifyEmailSent("mail/invoice-overdue-reminder", testBuyer.getEmail(), "Hóa đơn #" + testInvoice.getInvoiceNumber() + " đã quá hạn");
        }

        @Test
        @DisplayName("Send Overdue Invoice Admin Email - Success")
        void sendOverdueInvoiceAdminEmail_success() throws MessagingException {
            when(thymeleafTemplateEngine.process(eq("mail/invoice-overdue-admin-alert"), any(Context.class))).thenReturn("<html>Admin Alert</html>");
            emailService.sendOverdueInvoiceAdminEmail(testInvoice, List.of(testAdmin));
            verifyEmailSent("mail/invoice-overdue-admin-alert", testAdmin.getEmail(), "Cảnh báo: Hóa đơn #" + testInvoice.getInvoiceNumber() + " đã quá hạn");
        }
    ...
    class SendHtmlEmailHelperTests {
        ...
        void sendHtmlEmail_whenMailSendException_shouldLogError() { // Bỏ throws MessagingException
            when(thymeleafTemplateEngine.process(anyString(), any(Context.class))).thenReturn("<html></html>");
