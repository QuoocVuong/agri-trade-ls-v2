...
class NotificationServiceImplTest {
    ...
    class SendNotificationTests {
        ...
        void sendProductApprovedNotification_whenFarmerIsNull_shouldLogAndReturn() {
            ...
            verify(inAppNotificationService, never()).createAndSendInAppNotification(any(), anyString(), any(), anyString());
        }

//        @Test
//        @DisplayName("Send New Chat Message Notification")
//        void sendNewChatMessageNotification_shouldCallInAppService() {
//            notificationService.sendNewChatMessageNotification(testBuyer, testFarmer, 123L); // roomId 123
//
//            verify(inAppNotificationService).createAndSendInAppNotification(
//                    eq(testBuyer),
...
//        }
    ...
    class ManageInAppNotifications {
        ...
        void getMyUnreadNotificationCount_success() {
            when(notificationRepository.countByRecipientIdAndIsReadFalse(testUser.getId())).thenReturn(5L);
        ...
        void markNotificationAsRead_success() {
            ...
            when(notificationRepository.markAsRead(eq(notificationId), eq(testUser.getId()), any(LocalDateTime.class)))
                    .thenReturn(1); // 1 row updated

            notificationService.markNotificationAsRead(authentication, notificationId);
            verify(notificationRepository).markAsRead(eq(notificationId), eq(testUser.getId()), any(LocalDateTime.class));
        }
        ...
        void markNotificationAsRead_notFoundOrNotOwned_doesNotThrow() {
            ...
            assertDoesNotThrow(() -> notificationService.markNotificationAsRead(authentication, notificationId));
            verify(notificationRepository).markAsRead(eq(notificationId), eq(testUser.getId()), any(LocalDateTime.class));
        }
        ...
        void markAllMyNotificationsAsRead_success() {
            when(notificationRepository.markAllAsReadForRecipient(eq(testUser.getId()), any(LocalDateTime.class)))
                    .thenReturn(3); // 3 rows updated
            notificationService.markAllMyNotificationsAsRead(authentication);
            verify(notificationRepository).markAllAsReadForRecipient(eq(testUser.getId()), any(LocalDateTime.class));
        }
        ...
        void deleteNotification_notOwned_throwsAccessDenied() {
            ...
            User anotherUser = new User(); anotherUser.setId(99L);
            Notification notification = new Notification(anotherUser, "Msg", NotificationType.OTHER, null);
            notification.setId(notificationId);
    ...
    class UserEventNotifications {
        ...
        // ... Thêm test cho sendPasswordChangedNotification, sendAccountStatusUpdateNotification, sendRolesUpdateNotification ...
    }
    ...
    class InteractionEventNotifications {
        ...
        void sendNewFollowerNotification_callsInAppService() {
            notificationService.sendNewFollowerNotification(testFarmer, testBuyer); // Buyer follows Farmer
            verify(inAppNotificationService).createAndSendInAppNotification(
                    eq(testFarmer),
        ...
        void sendReviewApprovedNotification_callsInAppService() {
            ...
            verify(inAppNotificationService).createAndSendInAppNotification(
                    ...
                    contains("Đánh giá của bạn cho sản phẩm '" + testProduct.getName() + "' đã được duyệt"),
    ...
    class FarmerProfileEventNotifications {
        ...
        void sendFarmerProfileApprovedNotification_whenProfileNull_shouldLogAndReturn() {
            ...
            verify(inAppNotificationService, never()).createAndSendInAppNotification(any(), any(), any(), any());
        }
    ...
    class InvoiceEventNotifications {
        ...
        void sendOverdueInvoiceNotificationToAdmin_whenNoAdmins_shouldLogAndReturn() {
            when(userRepository.findByRoles_Name(RoleType.ROLE_ADMIN)).thenReturn(Collections.emptyList());
