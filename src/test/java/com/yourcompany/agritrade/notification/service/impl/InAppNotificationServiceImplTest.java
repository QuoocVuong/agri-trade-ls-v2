...
class InAppNotificationServiceImplTest {
    ...
    void createAndSendInAppNotification_whenValidInput_shouldSaveAndSendViaWebSocket() {
        ...
        when(notificationMapper.toNotificationResponse(savedNotification)).thenReturn(notificationResponseDto);
        doNothing().when(messagingTemplate).convertAndSend(anyString(), any(NotificationResponse.class));
    ...
    void createAndSendInAppNotification_whenWebSocketSendFails_shouldLogError() {
        ...
        when(notificationRepository.save(any(Notification.class))).thenReturn(savedNotification);
        when(notificationMapper.toNotificationResponse(savedNotification)).thenReturn(notificationResponseDto);
        doThrow(wsException).when(messagingTemplate).convertAndSend(anyString(), any(NotificationResponse.class));
