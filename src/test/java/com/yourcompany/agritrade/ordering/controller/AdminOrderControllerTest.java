...
class AdminOrderControllerTest {
    ...
    class GetAllOrdersTests {
        ...
        void getAllOrders_withFilters_success() throws Exception {
            when(orderService.getAllOrdersForAdmin(eq("keyword"), eq(OrderStatus.PENDING), eq(PaymentMethod.COD), eq(PaymentStatus.PENDING), eq(OrderType.B2B), eq(1L), eq(2L), any(Pageable.class)))
        ...
        void getAllOrders_noFilters_success() throws Exception {
            when(orderService.getAllOrdersForAdmin(isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), any(Pageable.class)))
                    .thenReturn(orderSummaryPage);
        ...
        void getAllOrders_invalidStatus_callsServiceWithNullStatus() throws Exception {
            ...
            verify(orderService).getAllOrdersForAdmin(isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), any(Pageable.class));
        }
    }
    ...
    class UpdateOrderStatusByAdminTests {
        ...
        void updateOrderStatusByAdmin_success() throws Exception {
            ...
            when(orderService.updateOrderStatus(any(Authentication.class), eq(orderId), any(OrderStatusUpdateRequest.class)))
    ...
    class CancelOrderByAdminTests {
        ...
        void cancelOrderByAdmin_success() throws Exception {
            ...
            when(orderService.cancelOrder(any(Authentication.class), eq(orderId))).thenReturn(orderResponse);
    ...
    class ConfirmBankTransferTests {
        @Test
        @DisplayName("POST /api/admin/orders/{orderId}/confirm-bank-transfer - Thành công với mã giao dịch")
        void confirmBankTransfer_withTransactionCode_success() throws Exception {
            ...
            mockMvc.perform(post("/api/admin/orders/{orderId}/confirm-bank-transfer", orderId)
                            .contentType(MediaType.APPLICATION_JSON)
        ...
        @Test
        @DisplayName("POST /api/admin/orders/{orderId}/confirm-bank-transfer - Thành công không có payload")
        ...
        @Test
        @DisplayName("POST /api/admin/orders/{orderId}/confirm-bank-transfer - Đơn hàng không phải chuyển khoản")
    ...
    class ConfirmOrderPaymentTests {
        ...
        void confirmOrderPayment_success() throws Exception {
            ...
            when(orderService.confirmOrderPaymentByAdmin(eq(orderId), eq(paymentMethod), eq("REF_ADMIN_001"), eq("Admin confirmed")))
                    .thenReturn(orderResponse);

            mockMvc.perform(post("/api/admin/orders/{orderId}/confirm-payment", orderId)
                            .param("paymentMethod", paymentMethod.name())
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(payload)))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success", is(true)))
                    .andExpect(jsonPath("$.message", is("Xác nhận thanh toán cho đơn hàng #" + orderId + " thành công.")))
                    .andExpect(jsonPath("$.data.status", is(OrderStatus.CONFIRMED.name())));
        }
