...
class CartControllerTest {
    ...
    void setUp() {
        ...
        cartValidationResponse = new CartValidationResponse(true, List.of("Giỏ hàng hợp lệ"), Collections.emptyList());
    }
    ...
    class AddItemToCartTests {
        ...
        void addItemToCart_productNotFound_throwsBadRequest() throws Exception {
            when(cartService.addItem(any(Authentication.class), any(CartItemRequest.class)))
                    .thenThrow(new ResourceNotFoundException("Product", "id", cartItemRequest.getProductId()));
            ...
            mockMvc.perform(post("/api/cart/items")
                    ...
                    .andExpect(jsonPath("$.message", is("Product not found with id : '" + cartItemRequest.getProductId() + "'")));
        }
    ...
    class UpdateCartItemQuantityTests {
        ...
        void updateCartItemQuantity_success() throws Exception {
            Long cartItemId = 1L;
            when(cartService.updateItemQuantity(any(Authentication.class), eq(cartItemId), any(CartItemUpdateRequest.class)))
                    .thenReturn(cartItemResponse);
    ...
    class RemoveCartItemTests {
        ...
        void removeCartItem_itemNotFound_throwsNotFound() throws Exception {
            ...
            doThrow(new ResourceNotFoundException("Cart Item", "id", cartItemId))
                    .when(cartService).removeItem(any(Authentication.class), eq(cartItemId));
    ...
    class ValidateCartTests {
        ...
        void validateCart_invalidCartWithAdjustments_success() throws Exception {
            CartValidationResponse invalidResponse = new CartValidationResponse(
                    false,
                    List.of("Số lượng sản phẩm X đã được cập nhật."),
                    List.of(new CartAdjustmentInfo(10L, "Sản phẩm X", "Số lượng đã được cập nhật thành 5 do thay đổi tồn kho.", "ADJUSTED"))
            );
            when(cartService.validateCartForCheckout(any(Authentication.class)))
