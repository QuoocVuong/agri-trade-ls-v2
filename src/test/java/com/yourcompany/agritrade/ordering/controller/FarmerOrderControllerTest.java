...
class FarmerOrderControllerTest {
    ...
    class GetMyOrdersAsFarmerTests {
        ...
        void getMyOrdersAsFarmer_withFilters_success() throws Exception {
            when(orderService.getMyOrdersAsFarmer(any(Authentication.class), eq("keyword"), eq(OrderStatus.PENDING), any(Pageable.class)))
        ...
        void getMyOrdersAsFarmer_noFilters_success() throws Exception {
            when(orderService.getMyOrdersAsFarmer(any(Authentication.class), isNull(), isNull(), any(Pageable.class)))
                    .thenReturn(orderSummaryPage);
        ...
        void getMyOrdersAsFarmer_invalidStatus_callsServiceWithNullStatus() throws Exception {
            ...
            verify(orderService).getMyOrdersAsFarmer(any(Authentication.class), isNull(), isNull(), any(Pageable.class));
        }
    }
    ...
    class GetMyOrderDetailsAsFarmerTests {
        ...
        void getMyOrderDetailsAsFarmer_success() throws Exception {
            Long orderId = 1L;
            when(orderService.getOrderDetails(any(Authentication.class), eq(orderId))).thenReturn(orderResponse);

            mockMvc.perform(get("/api/farmer/orders/{orderId}", orderId))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success", is(true)))
    ...
    class UpdateMyOrderStatusTests {
        ...
        void updateMyOrderStatus_success() throws Exception {
            ...
            mockMvc.perform(put("/api/farmer/orders/{orderId}/status", orderId)
        ...
        void updateMyOrderStatus_invalidRequest_throwsBadRequest() throws Exception {
            ...
            mockMvc.perform(put("/api/farmer/orders/{orderId}/status", orderId)
                    ...
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.success", is(false)))
                    .andExpect(jsonPath("$.message", is("Validation Failed"))) // Hoặc thông báo lỗi cụ thể từ GlobalExceptionHandler
