...
class OrderControllerTest {
  ...
  class GetMyOrdersBuyerTests {
    ...
    void getMyOrdersAsBuyer_withFilters_success() throws Exception {
      when(orderService.getMyOrdersAsBuyer(
  ...
  class GetOrderDetailsTests {
    ...
    void getMyOrderDetailsById_success() throws Exception {
      when(orderService.getOrderDetails(any(Authentication.class), eq(1L)))
    ...
    void getMyOrderDetailsById_notFound() throws Exception {
      ...
      mockMvc
          .perform(get("/api/orders/{orderId}", 99L))
    ...
    void getMyOrderDetailsByCode_success() throws Exception {
      when(orderService.getOrderDetailsByCode(any(Authentication.class), eq("ORD123")))
          .thenReturn(orderResponse);

      mockMvc
          .perform(get("/api/orders/code/{orderCode}", "ORD123"))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)))
          .andExpect(jsonPath("$.data.orderCode", is(orderResponse.getOrderCode())));
    }
  ...
  class CancelOrderTests {
    ...
    void cancelMyOrder_success() throws Exception {
      ...
      mockMvc
          .perform(post("/api/orders/{orderId}/cancel", 1L))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)))
    ...
    void calculateOrderTotals_success() throws Exception {
      ...
      mockMvc
          ...
          .andExpect(
              jsonPath(
                  "$.data.totalAmount",
                  is(orderCalculationResponse.getTotalAmount().doubleValue())));
    }
  ...
  class CreatePaymentUrlTests {
    ...
    void createPaymentUrl_vnPay_success() throws Exception {
      when(orderService.createPaymentUrl(
              any(Authentication.class),
              eq(1L),
      ...
      mockMvc
          .perform(
              post("/api/orders/{orderId}/create-payment-url", 1L).param("paymentMethod", "VNPAY"))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)))
    ...
    void createPaymentUrl_unsupportedMethod_throwsBadRequest() throws Exception {
      when(orderService.createPaymentUrl(
              ...
              any(HttpServletRequest.class)))
          .thenThrow(
              new BadRequestException("Payment method COD is not supported for URL creation."));

      mockMvc
          .perform(
              post("/api/orders/{orderId}/create-payment-url", 1L).param("paymentMethod", "COD"))
          .andExpect(status().isBadRequest())
          .andExpect(jsonPath("$.success", is(false)))
          .andExpect(
              jsonPath("$.message", is("Payment method COD is not supported for URL creation.")));
    }
  }
  ...
  class GetBankTransferInfoTests {
    ...
    void getBankTransferInfo_success() throws Exception {
      ...
      mockMvc
          .perform(get("/api/orders/{orderId}/bank-transfer-info", 1L))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)))
          .andExpect(
              jsonPath("$.data.accountNumber", is(bankTransferInfoResponse.getAccountNumber())));
    }
