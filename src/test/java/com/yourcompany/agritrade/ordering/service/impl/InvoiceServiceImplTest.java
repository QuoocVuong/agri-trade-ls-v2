...
import com.yourcompany.agritrade.usermanagement.domain.User; // Thêm import này
...
//        testOrder.setFarmer(testFarmer); // Gán farmer cho order
    ...
    class GetOrCreateInvoiceTests {
        ...
        @Test
        @DisplayName("Get Or Create - Invoice Not Exists, Payment Method COD, Payment PENDING - Creates New ISSUED Invoice")
        void getOrCreateInvoiceForOrder_whenInvoiceNotExistsAndCodPending_shouldCreateNewIssuedInvoice() {
            testOrder.setPaymentMethod(PaymentMethod.COD);
            testOrder.setPaymentStatus(PaymentStatus.PENDING);
        ...
        @Test
        @DisplayName("Get Or Create - Invoice Not Exists, Payment Method COD, Payment PAID - Creates New PAID Invoice")
        ...
        @Test
        @DisplayName("Get Or Create - Invoice Not Exists, Payment Method INVOICE - Creates New ISSUED Invoice with DueDate")
        void getOrCreateInvoiceForOrder_whenInvoiceNotExistsAndInvoiceMethod_shouldCreateNewIssuedInvoiceWithDueDate() {
            ...
            assertEquals(LocalDate.now().plusDays(7), result.getDueDate()); // 7 là giá trị invoicePaymentTermsDays đã set
            verify(invoiceRepository).save(any(Invoice.class));
        }
    }
    ...
    class GenerateInvoicePdfTests {
        ...
        void generateInvoicePdf_whenOrderNotFoundForInvoice_shouldThrowResourceNotFoundException() {
            ...
            when(orderRepository.findByIdWithDetails(testOrder.getId())).thenReturn(Optional.empty());
    ...
    class AdminGetAllInvoicesTests {
        ...
        void getAllInvoices_noFilters_shouldReturnPageOfSummaries() {
            ...
            Page<InvoiceSummaryResponse> result = invoiceService.getAllInvoices(null, null, pageable);

            assertNotNull(result);
            assertEquals(1, result.getTotalElements());
            assertEquals(summaryResponse.getInvoiceNumber(), result.getContent().get(0).getInvoiceNumber());
        ...
        void getAllInvoices_withStatusFilter_shouldCallRepositoryWithCorrectSpec() {
            ...
            invoiceService.getAllInvoices(status, null, pageable);
        ...
        void getAllInvoices_withKeywordFilter_shouldCallRepositoryWithCorrectSpec() {
            ...
            invoiceService.getAllInvoices(null, keyword, pageable);
