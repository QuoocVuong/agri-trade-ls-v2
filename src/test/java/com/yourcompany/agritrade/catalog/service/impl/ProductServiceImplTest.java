...
import com.yourcompany.agritrade.catalog.repository.ProductPricingTierRepository;
...
class ProductServiceImplTest {
    ...
    @Mock private ProductPricingTierRepository pricingTierRepository;
    @Mock private ProductMapper productMapper;
    @Mock private ProductImageMapper productImageMapper;
    @Mock private ProductPricingTierMapper productPricingTierMapper;
    ...
    private ProductSummaryResponse productSummaryResponse, productSummaryResponse2; // Thêm productSummaryResponse2
    ...
    void setUp() {
        ...
        productEntity.setImages(new HashSet<>(Set.of(new ProductImage(1L, productEntity, "http://signed-url.com/images/ca-rot.jpg", true, 0, "images/ca-rot.jpg", LocalDateTime.now()))));
        productEntity.setPricingTiers(new HashSet<>());
        ...
        productEntity2.setPricingTiers(new HashSet<>());
    ...
    class FarmerProductManagement {
        ...
        void getMyProducts_withKeywordAndStatus_shouldReturnFilteredProducts() {
            ...
            lenient().when(fileStorageService.getFileUrl(anyString())).thenReturn("http://signed-url.com/some-image.jpg");
            ...
            Page<ProductSummaryResponse> result = productService.getMyProducts(authentication, keyword, status, pageable);
...
//            when(productRepository.findTopNByCategoryIdAndIdNotAndStatus(any(), anyLong(), eq(ProductStatus.PUBLISHED), any(Pageable.class)))
//                    .thenReturn(Collections.emptyList());
//            when(productRepository.findTopNByFarmerIdAndIdNotInAndStatus(anyLong(), anyList(), eq(ProductStatus.PUBLISHED), any(Pageable.class)))
//                    .thenReturn(Collections.emptyList());


            ProductDetailResponse result = productService.getMyProductById(authentication, productEntity.getId());
        ...
        void getMyProductById_whenProductNotFound_shouldThrowResourceNotFoundException() {
            when(productRepository.findByIdAndFarmerId(99L, farmerUser.getId())).thenReturn(Optional.empty());
        ...
        void updateMyProduct_whenSlugChanges_shouldGenerateNewUniqueSlug() {
            ...
            when(productRepository.findByIdAndFarmerId(productEntity.getId(), farmerUser.getId())).thenReturn(Optional.of(productEntity));
            when(productRepository.existsBySlugAndIdNot(eq(newExpectedSlug), eq(productEntity.getId()))).thenReturn(false); // Slug mới là unique
...
//            when(productRepository.findByIdWithDetails(productEntity.getId())).thenReturn(Optional.of(productEntity));
        ...
        void updateMyProduct_whenCategoryChanges_shouldUpdateCategory() {
            ...
            newCategory.setId(200); newCategory.setName("Củ Quả");
            ProductRequest updateRequest = new ProductRequest();
...
//            when(productRepository.findByIdWithDetails(productEntity.getId())).thenReturn(Optional.of(productEntity));
        ...
        void updateMyProduct_whenImagesUpdated_shouldReflectChanges() {
            productEntity.setStatus(ProductStatus.PUBLISHED);
            ProductImage existingImageEntity = new ProductImage(1L, productEntity, "url1", true, 0, "path1", LocalDateTime.now());
            productEntity.setImages(new HashSet<>(Set.of(existingImageEntity)));

            ProductImageRequest newImageReq = new ProductImageRequest();
            newImageReq.setBlobPath("path2"); newImageReq.setImageUrl("url2"); newImageReq.setIsDefault(false); newImageReq.setDisplayOrder(1);
            ...
            updatedExistingImageReq.setBlobPath("path1"); updatedExistingImageReq.setImageUrl("url1");
            updatedExistingImageReq.setIsDefault(true); updatedExistingImageReq.setDisplayOrder(0);
            ...
            when(productImageMapper.requestToProductImage(eq(newImageReq))).thenReturn(newImageEntityMappedFromRequest);
            ...
            imageEntityMappedFromUpdatedExistingReq.setDisplayOrder(updatedExistingImageReq.getDisplayOrder());
            when(productImageMapper.requestToProductImage(eq(updatedExistingImageReq))).thenReturn(imageEntityMappedFromUpdatedExistingReq);

            // --- Mock các repository và mapper khác ---
//            when(productRepository.findByIdAndFarmerId(productEntity.getId(), farmerUser.getId())).thenReturn(Optional.of(productEntity));
            when(productRepository.save(any(Product.class))).thenAnswer(invocation -> invocation.getArgument(0));

            // Mock cho lời gọi getMyProductById -> findMyProductById -> productRepository.findByIdAndFarmerId
            ...
            when(productMapper.toProductDetailResponse(eq(productEntity))).thenAnswer(invocation -> {
                ...
                if (p.getImages() != null) {
                    res.setImages(p.getImages().stream().map(img -> {
                        ...
                        lenient().when(fileStorageService.getFileUrl(img.getBlobPath())).thenReturn("mockedImageUrl/" + img.getBlobPath());
                        imgRes.setImageUrl(fileStorageService.getFileUrl(img.getBlobPath()));
            ...
            // --- Act ---
            ProductDetailResponse result = productService.updateMyProduct(authentication, productEntity.getId(), updateRequest);
            ...
            assertTrue(finalImagesInProduct.stream().anyMatch(img ->
        ...
        @Test
        @DisplayName("Update My Product - B2B Pricing Tier Updates")
        void updateMyProduct_whenB2BPricingTiersUpdated_shouldReflectChanges() {
            productEntity.setStatus(ProductStatus.PUBLISHED);
            productEntity.setB2bEnabled(true);

            ProductPricingTierRequest tierReq1 = new ProductPricingTierRequest();
            tierReq1.setMinQuantity(10); tierReq1.setPricePerUnit(new BigDecimal("12000"));
            ProductPricingTierRequest tierReq2 = new ProductPricingTierRequest();
            tierReq2.setMinQuantity(50); tierReq2.setPricePerUnit(new BigDecimal("10000"));

            ProductRequest updateRequest = new ProductRequest();
            updateRequest.setCategoryId(categoryEntity.getId());
            updateRequest.setB2bEnabled(true);
            updateRequest.setPricingTiers(List.of(tierReq1, tierReq2));

            // Mock cho ProductPricingTierMapper
            ProductPricingTier tierEntity1 = new ProductPricingTier(); tierEntity1.setMinQuantity(10); tierEntity1.setPricePerUnit(new BigDecimal("12000"));
            ProductPricingTier tierEntity2 = new ProductPricingTier(); tierEntity2.setMinQuantity(50); tierEntity2.setPricePerUnit(new BigDecimal("10000"));
            when(productPricingTierMapper.requestToProductPricingTier(tierReq1)).thenReturn(tierEntity1);
            when(productPricingTierMapper.requestToProductPricingTier(tierReq2)).thenReturn(tierEntity2);
...
//            when(productRepository.findByIdWithDetails(productEntity.getId())).thenReturn(Optional.of(productEntity));
            ...
            assertEquals(2, productEntity.getPricingTiers().size());
            assertTrue(productEntity.getPricingTiers().stream().anyMatch(t -> t.getMinQuantity() == 10));
//            verify(pricingTierRepository, atLeastOnce()).deleteByProductId(productEntity.getId()); // Kiểm tra tier cũ bị xóa
        }
    ...
    class AdminProductManagement {
        ...
        void getAllProductsForAdmin_withFilters_shouldReturnFilteredPage() {
            ...
            when(productRepository.findAll(any(Specification.class), eq(pageable))).thenReturn(productPage);
            when(productMapper.toProductSummaryResponse(productEntity)).thenReturn(productSummaryResponse);
            lenient().when(fileStorageService.getFileUrl(anyString())).thenReturn("http://signed-url.com/image.jpg");


            Page<ProductSummaryResponse> result = productService.getAllProductsForAdmin(
        ...
        void getProductByIdForAdmin_whenProductNotFound_shouldThrowResourceNotFound() {
            ...
            assertThrows(ResourceNotFoundException.class, () -> productService.getProductByIdForAdmin(99L));
        }
    }
    ...
    class PublicProductViewing {
        ...
        void getPublicProductById_whenProductPublished_shouldReturnDetails() {
            ...
            when(productRepository.findOne(any(Specification.class))).thenReturn(Optional.of(productEntity));
            when(productMapper.toProductDetailResponse(productEntity)).thenReturn(productDetailResponse);
        ...
        void getPublicProductById_whenProductNotPublished_shouldThrowResourceNotFound() {
            productEntity.setStatus(ProductStatus.PENDING_APPROVAL);
            when(productRepository.findOne(any(Specification.class))).thenReturn(Optional.empty()); // Spec sẽ không tìm thấy
        ...
        void getPublicProductsByFarmerId_whenFarmerHasPublishedProducts_shouldReturnPage() {
            ...
            when(userRepository.existsByIdAndRoles_Name(farmerUser.getId(), RoleType.ROLE_FARMER)).thenReturn(true);
            when(productRepository.findByFarmerIdAndStatus(farmerUser.getId(), ProductStatus.PUBLISHED, pageable))
                    .thenReturn(productPage);
            when(productMapper.toProductSummaryResponse(productEntity)).thenReturn(productSummaryResponse);
            lenient().when(fileStorageService.getFileUrl(anyString())).thenReturn("http://signed-url.com/image.jpg");


            Page<ProductSummaryResponse> result = productService.getPublicProductsByFarmerId(farmerUser.getId(), pageable);

            assertNotNull(result);
            assertEquals(1, result.getTotalElements());
            assertEquals(productSummaryResponse.getName(), result.getContent().get(0).getName());
            verify(productRepository).findByFarmerIdAndStatus(farmerUser.getId(), ProductStatus.PUBLISHED, pageable);
        }
    }
    ...
    class ErrorHandlingAndEdgeCases {
        ...
        void createMyProduct_whenFileStorageServiceFails_shouldStillSaveProductWithoutImageErrorPropagation() {
            ...
            when(productRepository.save(any(Product.class))).thenReturn(productEntity); // Product vẫn được lưu
            when(productRepository.findByIdWithDetails(productEntity.getId())).thenReturn(Optional.of(productEntity));
            when(productMapper.toProductDetailResponse(productEntity)).thenReturn(productDetailResponse);
            ...
            lenient().when(fileStorageService.getFileUrl(anyString())).thenThrow(new RuntimeException("Simulated storage error"));
        ...
        void updateMyProduct_whenSlugConflict_shouldThrowBadRequest() {
            ...
            when(productRepository.findByIdAndFarmerId(productEntity.getId(), farmerUser.getId())).thenReturn(Optional.of(productEntity));
            when(productRepository.existsBySlugAndIdNot(eq("conflicting-name"), eq(productEntity.getId()))).thenReturn(true); // Slug bị trùng
