...
class CategoryServiceImplTest {
    ...
    @Spy
    private Slugify slugify = Slugify.builder().build(); // Sử dụng instance thật của Slugify
    ...
    void setUp() {
        ...
        categoryResponse1.setImageUrl("http://example.com/images/rau-an-la.jpg"); // Giả sử mapper đã xử lý
    ...
    class GetCategoryTests {
        ...
        void getAllCategoriesForDropdown_shouldReturnSortedCategories() {
            ...
            when(categoryMapper.toCategoryResponseList(List.of(category2, category1)))
                    .thenReturn(List.of(categoryResponse2, categoryResponse1)); // Mapper giữ nguyên thứ tự từ repo
        ...
        void getCategoryTree_shouldReturnRootCategoriesWithChildren() {
            ...
            Category rootCategory2 = new Category(); rootCategory2.setId(20); rootCategory2.setName("Củ Quả");
            CategoryResponse rootResponse2 = new CategoryResponse(); rootResponse2.setId(20); rootResponse2.setName("Củ Quả");

            when(categoryRepository.findByParentIsNull()).thenReturn(List.of(parentCategory, rootCategory2));
            when(categoryMapper.toCategoryResponse(parentCategory)).thenReturn(parentCategoryResponse); // Giả sử mapper xử lý children
    ...
    class CreateCategoryTests {
        ...
        void createCategory_successWithAutoSlugNoParent() {
            ...
            when(categoryRepository.existsBySlug(slugify.slugify(categoryRequest.getName()))).thenReturn(false);
    ...
    class UpdateCategoryTests {
        ...
        void updateCategory_successWithAllChanges() {
            ...
            when(categoryRepository.existsBySlugAndIdNot("rau-sach-moi-slug", categoryIdToUpdate)).thenReturn(false);
            // categoryMapper.updateCategoryFromRequest là void
            doNothing().when(categoryMapper).updateCategoryFromRequest(eq(categoryRequest), any(Category.class));
            when(categoryRepository.save(any(Category.class))).thenAnswer(invocation -> invocation.getArgument(0)); // Trả về chính nó
    ...
    class DeleteCategoryTests {
        ...
        void deleteCategory_successNoProductsOrChildren_deletesImage() {
            Integer categoryIdToDelete = category2.getId(); // category2 không có parent, children, product
