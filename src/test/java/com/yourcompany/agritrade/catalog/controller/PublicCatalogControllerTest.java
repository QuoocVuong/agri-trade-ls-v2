...
import com.yourcompany.agritrade.config.TestSecurityConfig; // THÊM IMPORT NÀY
...
class PublicCatalogControllerTest {
    ...
    class CategoryEndpoints {
        ...
        void getCategoryTree_returnsCategoryTree() throws Exception {
            when(categoryService.getCategoryTree()).thenReturn(List.of(categoryResponse)); // Giả sử tree đơn giản
        ...
        void getCategoryBySlug_categoryFound_returnsCategory() throws Exception {
            ...
            mockMvc.perform(get("/api/public/categories/{slug}", slug)
                            .contentType(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success", is(true)))
    ...
    class ProductEndpoints {
        ...
        void searchProducts_withAllFilters_returnsPaginatedProducts() throws Exception {
            ...
            when(productService.searchPublicProducts(
                    eq(keyword), eq(categoryId), eq(provinceCode), eq(minPrice), eq(maxPrice), eq(minRating), any(Pageable.class)))
            ...
            verify(productService).searchPublicProducts(eq(keyword), eq(categoryId), eq(provinceCode), eq(minPrice), eq(maxPrice), eq(minRating), any(Pageable.class));
        }
        ...
        void getProductBySlug_productFound_returnsProductDetail() throws Exception {
            ...
            mockMvc.perform(get("/api/public/products/{slug}", slug)
                            .contentType(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success", is(true)))
                    .andExpect(jsonPath("$.data.slug", is(slug)));
        ...
        void getProductById_productFound_returnsProductDetail() throws Exception {
            ...
            mockMvc.perform(get("/api/public/products/id/{id}", productId)
                            .contentType(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success", is(true)))
        ...
        void getProductsByFarmerPublic_success_returnsPaginatedProducts() throws Exception {
            ...
            mockMvc.perform(get("/api/public/farmer/{farmerId}/products", farmerId)
                            .param("page", "0")
                            .param("size", "8")
                            .contentType(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk())
