...
class AdminCatalogControllerTest {
    ...
    class CategoryManagementTests {
        ...
        void updateCategory_success() throws Exception {
            when(categoryService.updateCategory(eq(1), any(CategoryRequest.class))).thenReturn(categoryResponse);

            mockMvc.perform(put("/api/admin/categories/{id}", 1)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(categoryRequest)))
        ...
        void updateCategory_notFound() throws Exception {
            ...
            mockMvc.perform(put("/api/admin/categories/{id}", 99)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(categoryRequest)))
        ...
        void deleteCategory_conflict() throws Exception {
            doThrow(new BadRequestException("Category is in use")).when(categoryService).deleteCategory(1);
    ...
    class ProductManagementTests {
        ...
        void approveProduct_success() throws Exception {
            ...
            mockMvc.perform(post("/api/admin/products/{id}/approve", 1L))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success", is(true)))
        ...
        void rejectProduct_withReason_success() throws Exception {
            ...
            when(productService.rejectProduct(eq(1L), eq("Not suitable"))).thenReturn(productDetailResponse);

            mockMvc.perform(post("/api/admin/products/{id}/reject", 1L)
                            .contentType(MediaType.APPLICATION_JSON)
        ...
        void forceDeleteProduct_success() throws Exception {
            ...
            mockMvc.perform(delete("/api/admin/products/{id}/force", 1L))
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.success", is(true)))
