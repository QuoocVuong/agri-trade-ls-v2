...
class FileControllerTest {
  ...
  class FileUploadTests {
    ...
    void uploadFile_success() throws Exception {
      ...
      mockMvc
          ...
          .andExpect(
              jsonPath(
                  "$.message",
                  is("File uploaded successfully"))) // Giữ tiếng Anh nếu API trả về tiếng Anh
    ...
    void uploadMultipleFiles_success() throws Exception {
      ...
      mockMvc
          .perform(
              MockMvcRequestBuilders.multipart("/api/files/uploadMultiple")
  ...
  class FileDownloadTests {
    ...
    void downloadFile_png_success() throws Exception {
      ...
      // when(resource.getFile()).thenThrow(new IOException("Simulated: Not a file system
      // resource"));
      ...
      mockMvc
          .perform(get("/api/files/download/" + blobPath))
          .andExpect(status().isOk())
          .andExpect(
              header()
                  .string(HttpHeaders.CONTENT_DISPOSITION, "inline; filename=\"test-image.png\""))
    ...
    void downloadFile_notFound_shouldReturn404() throws Exception {
      ...
      when(fileStorageService.loadAsResource(blobPath))
      ...
      mockMvc
          ...
          .andExpect(
              jsonPath(
                  "$.message", is("File not found"))); // Kiểm tra message từ GlobalExceptionHandler
    }
  }
  ...
  class FileDeletionTests {
    ...
    void deleteFile_success() throws Exception {
      ...
      mockMvc
          .perform(delete("/api/files/" + blobPath))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)))
    ...
    void deleteFile_notFound_shouldReturn404() throws Exception {
      ...
      doThrow(new StorageFileNotFoundException("File not found for deletion"))
  ...
  class FileDeletionPermissionTests {
    ...
    @DisplayName(
        "DELETE /api/files/** - Xóa Tệp - Được phép đối với người dùng có quyền MANAGE_OWN_FILES")
