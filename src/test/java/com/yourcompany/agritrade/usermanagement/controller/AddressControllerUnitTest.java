...
class AddressControllerUnitTest {
  ...
  void getMyAddresses_success_returnsOkWithAddressList() {
    ...
    ResponseEntity<ApiResponse<List<AddressResponse>>> responseEntity =
  ...
  void getMyAddressById_found_returnsOkWithAddress() {
    ...
    ResponseEntity<ApiResponse<AddressResponse>> responseEntity =
        addressController.getMyAddressById(authentication, 1L);
  ...
  void getMyAddressById_notFound_throwsResourceNotFound_handledByGlobalHandler() {
    ...
    when(addressService.getMyAddressById(authentication, 99L))
  ...
  void addMyAddress_success_returnsCreatedWithAddress() {
    when(addressService.addMyAddress(authentication, addressRequest))
        .thenReturn(addressResponseDto);

    ResponseEntity<ApiResponse<AddressResponse>> responseEntity =
        addressController.addMyAddress(authentication, addressRequest);
  ...
  void updateMyAddress_success_returnsOkWithUpdatedAddress() {
    when(addressService.updateMyAddress(authentication, 1L, addressRequest))
        .thenReturn(addressResponseDto);

    ResponseEntity<ApiResponse<AddressResponse>> responseEntity =
        addressController.updateMyAddress(authentication, 1L, addressRequest);
  ...
  void deleteMyAddress_success_returnsOk() {
    ...
    ResponseEntity<ApiResponse<Void>> responseEntity =
        addressController.deleteMyAddress(authentication, 1L);
  ...
  void setMyDefaultAddress_success_returnsOk() {
    ...
    ResponseEntity<ApiResponse<Void>> responseEntity =
        addressController.setMyDefaultAddress(authentication, 1L);
