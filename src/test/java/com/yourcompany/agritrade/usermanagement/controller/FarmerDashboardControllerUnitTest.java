...
class FarmerDashboardControllerUnitTest {
  ...
  @InjectMocks private FarmerDashboardController farmerDashboardController;
  ...
  void setUp() {
    ...
    chartData =
        Collections.singletonList(
            new FarmerChartDataResponse(LocalDate.now(), 10L)); // Ví dụ dữ liệu chart
  ...
  void getStats_success_returnsOkWithDashboardStats() {
    // Arrange
    when(dashboardService.getFarmerDashboardStats(authentication))
        .thenReturn(dashboardStatsResponse);
  ...
  void getRecentOrders_withDefaultLimit_callsServiceWithDefaultLimit() {
    ...
    when(dashboardService.getRecentFarmerOrders(authentication, defaultLimit))
        .thenReturn(recentOrders);

    // Act
    farmerDashboardController.getRecentOrders(
        authentication, defaultLimit); // Truyền giá trị mặc định
  ...
  void getTopProducts_success_returnsOkWithProductList() {
    // Arrange
    int limit = 3;
    when(dashboardService.getTopSellingFarmerProducts(authentication, limit))
  ...
  void getOrderCountChartData_success_returnsOkWithChartData() {
    ...
    when(dashboardService.getFarmerOrderCountChartData(authentication, startDate, endDate))
  ...
  void getRevenueChartData_success_returnsOkWithChartData() {
    ...
    LocalDate endDate = LocalDate.now();
    List<FarmerChartDataResponse> revenueData =
        Collections.singletonList(
            new FarmerChartDataResponse(LocalDate.now().toString(), new BigDecimal("500000")));
    when(dashboardService.getFarmerRevenueChartData(authentication, startDate, endDate))
  ...
  //     when(dashboardService.getFarmerDashboardStats(authentication)).thenThrow(new
