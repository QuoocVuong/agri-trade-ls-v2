...
class UserControllerTest {
  ...
  class AuthenticatedUserApis {
    ...
    void getCurrentUserProfile_success() throws Exception {
      when(userService.getCurrentUserProfile(any(Authentication.class)))
    ...
    void changePassword_success() throws Exception {
      doNothing()
          .when(userService)
          .changePassword(any(Authentication.class), any(PasswordChangeRequest.class));
    ...
    void updateMyProfile_success() throws Exception {
      when(userService.updateCurrentUserProfile(
              any(Authentication.class), any(UserUpdateRequest.class)))
  ...
  class AdminApis {
    ...
    void getAllUsersForAdmin_withFilters_success() throws Exception {
      when(adminUserService.getAllUsers(
              any(Pageable.class), eq(RoleType.ROLE_FARMER), eq("keyword"), eq(true)))
    ...
    void getAllUsersForAdmin_invalidRole_callsServiceWithNullRole() throws Exception {
      ...
      mockMvc
          .perform(get("/api/users").param("role", "INVALID_ROLE"))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)));
    ...
    void getUserProfileById_success() throws Exception {
      ...
      mockMvc
          .perform(get("/api/users/{id}/profile", userId))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)))
    ...
    void updateUserStatus_success() throws Exception {
      ...
      mockMvc
          .perform(
              put("/api/users/{id}/status", userId).param("isActive", String.valueOf(isActive)))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)))
