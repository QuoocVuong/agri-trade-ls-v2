...
class UserServiceImplTest {
    ...
    class RegistrationAndVerification {
        ...
        void registerUser_success() {
            ...
            verify(emailService).sendVerificationEmail(any(User.class), anyString(), contains("/auth/verify-email?token="));
        }
...
//            when(businessProfileRepository.findById(testUser.getId())).thenReturn(Optional.empty());
        ...
        void updateUserRoles_success() {
            ...
            when(roleRepository.findByName(RoleType.ROLE_CONSUMER)).thenReturn(Optional.of(consumerRole));
            when(userRepository.save(testUser)).thenReturn(testUser);
            when(userMapper.toUserResponse(testUser)).thenReturn(userResponseDto); // userResponseDto cần được cập nhật roles
    ...
    class PasswordResetFlow {
        ...
        void initiatePasswordReset_userExists_sendsEmail() {
            ...
            doNothing().when(emailService).sendPasswordResetEmail(any(User.class), anyString(), anyString());

            userService.initiatePasswordReset(email);

            assertNotNull(existingUser.getVerificationToken());
            assertNotNull(existingUser.getVerificationTokenExpiry());
            verify(emailService).sendPasswordResetEmail(eq(existingUser), eq(existingUser.getVerificationToken()), contains("/auth/reset-password?token="));
        }
    ...
    class PublicFarmerFeatures {
        ...
        void getFeaturedFarmers_success() {
            ...
            when(userRepository.findTopByRoles_NameOrderByFollowerCountDesc(RoleType.ROLE_FARMER, pageable))
                    .thenReturn(topFarmerUsers);
            when(farmerProfileRepository.findById(farmerUser.getId())).thenReturn(Optional.of(farmerProfileEntity));

            FarmerSummaryResponse summaryDto = new FarmerSummaryResponse(farmerUser.getId(), farmerProfileEntity.getFarmName(), farmerUser.getFullName(), farmerUser.getAvatarUrl(), farmerProfileEntity.getProvinceCode(), farmerUser.getFollowerCount());
            when(farmerSummaryMapper.toFarmerSummaryResponse(farmerUser, farmerProfileEntity)).thenReturn(summaryDto);
        ...
        void searchPublicFarmers_success() {
            ...
            when(farmerProfileRepository.findAll(any(Specification.class), eq(pageable))).thenReturn(profilePage);
            FarmerSummaryResponse summaryDto = new FarmerSummaryResponse(farmerUser.getId(), farmerProfileEntity.getFarmName(), farmerUser.getFullName(), farmerUser.getAvatarUrl(), farmerProfileEntity.getProvinceCode(), farmerUser.getFollowerCount());
            when(farmerSummaryMapper.toFarmerSummaryResponse(farmerUser, farmerProfileEntity)).thenReturn(summaryDto);


            Page<FarmerSummaryResponse> result = userService.searchPublicFarmers(keyword, provinceCode, pageable);
    ...
    class OAuthAndToken {
        ...
        void refreshToken_validToken_success() {
            ...
            Collection<? extends GrantedAuthority> authorities = Set.of(new SimpleGrantedAuthority(RoleType.ROLE_CONSUMER.name()));
            Authentication authForTokenGeneration = new UsernamePasswordAuthenticationToken(testUser.getEmail(), null, authorities);
            ...
            when(userMapper.toUserResponse(testUser)).thenReturn(userResponseDto); // userResponseDto cần có roles
