...
class FarmerProfileServiceImplUnitTest {
  ...
  void createOrUpdateFarmerProfile_createNewProfile_success() {
    ...
    savedNewProfile.setVerificationStatus(
        VerificationStatus.PENDING); // Service sẽ set PENDING khi tạo mới
    ...
    when(farmerProfileRepository.existsById(currentUser.getId()))
    ...
    assertEquals(
        currentUser,
        capturedProfile.getUser()); // Kiểm tra user đã được gán vào profile trước khi save
    assertEquals(VerificationStatus.PENDING, capturedProfile.getVerificationStatus());

    ArgumentCaptor<User> userCaptor = ArgumentCaptor.forClass(User.class);
    verify(userRepository).save(userCaptor.capture());
    assertTrue(
        userCaptor.getValue().getRoles().contains(farmerRole)); // Kiểm tra role FARMER đã được thêm
  ...
  void createOrUpdateFarmerProfile_updateExistingProfile_keepsVerificationStatus() {
    ...
    when(farmerProfileRepository.existsById(currentUser.getId()))
        .thenReturn(true); // Profile đã tồn tại
