...
//        @Test
//        @DisplayName("Update Role Permissions - Some Permissions Not Found - Throws ResourceNotFoundException")
//        void updateRolePermissions_whenSomePermissionsNotFound_shouldThrowResourceNotFoundException() {
//            Integer roleId = adminRoleEntity.getId();
//            Set<String> requestedPermissionNames = Set.of("USER_READ_ALL", "INVALID_PERMISSION");
//            RoleUpdateRequest request = new RoleUpdateRequest();
...
//                    () -> roleService.updateRolePermissions(roleId, request));
//
//            assertTrue(exception.getMessage().contains("Permission not found with names : '[INVALID_PERMISSION]'"));
//            verify(roleRepository, never()).save(any(Role.class));
//        }
        ...
        void updateRolePermissions_withEmptyPermissionList_shouldClearPermissions() {
            ...
            when(roleRepository.findById(roleId)).thenReturn(Optional.of(adminRoleEntity));
            when(permissionRepository.findByNameIn(Collections.emptySet())).thenReturn(Collections.emptySet());
            when(roleRepository.save(any(Role.class))).thenReturn(updatedRoleEntity);
            when(roleMapper.toRoleResponse(updatedRoleEntity)).thenReturn(expectedResponse);
