...
class DashboardServiceImplTest {
    ...
    private final List<OrderStatus> REVENUE_STATUSES = Arrays.asList(OrderStatus.SHIPPING, OrderStatus.DELIVERED);
    private final List<OrderStatus> PENDING_STATUSES = Arrays.asList(OrderStatus.PENDING, OrderStatus.CONFIRMED, OrderStatus.PROCESSING);
    ...
    class FarmerDashboard {
        ...
        void getFarmerDashboardStats_success() {
            ...
            when(orderRepository.countByFarmerIdAndCreatedAtBetween(eq(FARMER_ID), eq(todayStart), eq(todayEnd))).thenReturn(5L);
            when(orderRepository.countByFarmerIdAndCreatedAtBetween(eq(FARMER_ID), eq(monthStart), eq(todayEnd))).thenReturn(20L);
            when(orderRepository.sumTotalAmountByFarmerIdAndStatusInAndCreatedAtBetween(eq(FARMER_ID), eq(REVENUE_STATUSES), eq(todayStart), eq(todayEnd)))
                    .thenReturn(new BigDecimal("1500.00"));
            when(orderRepository.sumTotalAmountByFarmerIdAndStatusInAndCreatedAtBetween(eq(FARMER_ID), eq(REVENUE_STATUSES), eq(monthStart), eq(todayEnd)))
                    .thenReturn(new BigDecimal("10000.00"));
            when(orderRepository.countByFarmerIdAndStatusIn(eq(FARMER_ID), eq(PENDING_STATUSES))).thenReturn(3L);
            when(productRepository.countByFarmerIdAndStockQuantityLessThan(eq(FARMER_ID), anyInt())).thenReturn(2L);
            when(reviewRepository.countByProductFarmerIdAndStatus(eq(FARMER_ID), eq(ReviewStatus.PENDING))).thenReturn(1L);
        ...
        void getRecentFarmerOrders_success() {
            ...
            Order order1 = new Order(); order1.setId(101L);
            Order order2 = new Order(); order2.setId(102L);
            Page<Order> orderPage = new PageImpl<>(List.of(order1, order2), pageable, 2);
            OrderSummaryResponse summary1 = new OrderSummaryResponse(); summary1.setId(101L);
            OrderSummaryResponse summary2 = new OrderSummaryResponse(); summary2.setId(102L);

            when(orderRepository.findByFarmerIdWithDetails(FARMER_ID, pageable)).thenReturn(orderPage);
            when(orderMapper.toOrderSummaryResponseList(List.of(order1, order2))).thenReturn(List.of(summary1, summary2));

            List<OrderSummaryResponse> result = dashboardService.getRecentFarmerOrders(authentication, limit);
        ...
        void getTopSellingFarmerProducts_success() {
            ...
            TopProductResponse topProduct1 = new TopProductResponse(201L, "Product X", "prod-x", 10L, new BigDecimal("1000"));
            TopProductResponse topProduct2 = new TopProductResponse(202L, "Product Y", "prod-y", 8L, new BigDecimal("800"));
            List<TopProductResponse> topProductsFromRepo = List.of(topProduct1, topProduct2);

            when(productRepository.findTopSellingProductsByFarmerWithoutThumbnail(FARMER_ID, pageable)).thenReturn(topProductsFromRepo);
            // Giả sử productRepository.findById trả về product có ảnh để test logic gán thumbnail
            com.yourcompany.agritrade.catalog.domain.Product p1 = new com.yourcompany.agritrade.catalog.domain.Product();
            p1.setId(201L);
            ProductImage img1 = new ProductImage(); img1.setImageUrl("url1"); img1.setDefault(true);
            p1.setImages(new HashSet<>(Set.of(img1)));
            when(productRepository.findById(201L)).thenReturn(Optional.of(p1));

            List<TopProductResponse> result = dashboardService.getTopSellingFarmerProducts(authentication, limit);
        ...
        void getFarmerOrderCountChartData_success() {
            ...
            List<FarmerChartDataResponse> result = dashboardService.getFarmerOrderCountChartData(authentication, startDate, endDate);
        ...
        void getFarmerRevenueChartData_success() {
            ...
            when(orderRepository.sumRevenueByDayForFarmer(FARMER_ID, startDateTime, endDateTime, REVENUE_STATUSES))
                    .thenReturn(List.of(tuple1));

            List<FarmerChartDataResponse> result = dashboardService.getFarmerRevenueChartData(authentication, startDate, endDate);
    ...
    class AdminDashboard {
        ...
        void getAdminDashboardStats_success() {
            ...
            when(orderRepository.sumTotalAmountByStatusInAndCreatedAtBetween(eq(REVENUE_STATUSES), eq(todayStart), eq(todayEnd)))
                    .thenReturn(new BigDecimal("5000.00"));
            when(orderRepository.sumTotalAmountByStatusInAndCreatedAtBetween(eq(REVENUE_STATUSES), eq(monthStart), eq(todayEnd)))
            ...
            when(farmerProfileRepository.countByVerificationStatus(VerificationStatus.PENDING)).thenReturn(5L);
        ...
        void getDailyRevenueForAdminChart_success() {
            ...
            List<TimeSeriesDataPoint<BigDecimal>> result = dashboardService.getDailyRevenueForAdminChart(startDate, endDate);
        ...
        void getDailyOrderCountForAdminChart_success() {
            ...
            List<TimeSeriesDataPoint<Long>> result = dashboardService.getDailyOrderCountForAdminChart(startDate, endDate);
        ...
        void getRecentActivitiesForAdmin_success() {
            ...
            Order recentOrder = new Order(); recentOrder.setId(1L); recentOrder.setOrderCode("ORD-RECENT");
            recentOrder.setCreatedAt(LocalDateTime.now().minusHours(1));
            User buyerForRecentOrder = new User(); buyerForRecentOrder.setFullName("Recent Buyer");
            recentOrder.setBuyer(buyerForRecentOrder);

            User recentUser = new User(); recentUser.setId(10L); recentUser.setFullName("New User");
            recentUser.setEmail("new@example.com"); recentUser.setCreatedAt(LocalDateTime.now().minusMinutes(30));

            com.yourcompany.agritrade.interaction.domain.Review recentReview = new com.yourcompany.agritrade.interaction.domain.Review();
            recentReview.setId(5L);
            com.yourcompany.agritrade.catalog.domain.Product reviewedProduct = new com.yourcompany.agritrade.catalog.domain.Product();
            ...
            when(orderRepository.findTopNByOrderByCreatedAtDesc(pageable)).thenReturn(List.of(recentOrder));
            when(userRepository.findTopNByOrderByCreatedAtDesc(pageable)).thenReturn(List.of(recentUser));
            when(reviewRepository.findTopNByStatusOrderByCreatedAtDesc(ReviewStatus.PENDING, pageable)).thenReturn(List.of(recentReview));
            ...
            boolean hasOrderActivity = result.stream().anyMatch(r -> r.getType() == NotificationType.ORDER_PLACED);
            boolean hasUserActivity = result.stream().anyMatch(r -> r.getType() == NotificationType.WELCOME);
            boolean hasReviewActivity = result.stream().anyMatch(r -> r.getType() == NotificationType.REVIEW_PENDING);
        ...
        void getPendingApprovalCounts_success() {
            when(farmerProfileRepository.countByVerificationStatus(VerificationStatus.PENDING)).thenReturn(3L);
    ...
    void getUserFromAuthentication_whenUserNotFound_shouldThrowUsernameNotFoundException() {
        ...
        when(authentication.isAuthenticated()).thenReturn(true); // <<< QUAN TRỌNG: Đảm bảo user được coi là đã xác thực
        when(userRepository.findByEmail(unknownEmail)).thenReturn(Optional.empty()); // User không tồn tại

        // Gọi một phương thức bất kỳ trong service mà sử dụng getUserFromAuthentication
        assertThrows(UsernameNotFoundException.class, () -> dashboardService.getFarmerDashboardStats(authentication));
    }
    ...
    void getUserFromAuthentication_whenNotAuthenticated_shouldThrowAccessDeniedException() {
        ...
        assertThrows(AccessDeniedException.class, () -> dashboardService.getFarmerDashboardStats(authentication));
    }
}