...
class WebSocketChatControllerTest {
  ...
  @InjectMocks private WebSocketChatController webSocketChatController;
  ...
  void setUp() {
    ...
    mockAuthentication =
        new UsernamePasswordAuthenticationToken(TEST_USER_EMAIL, null, Collections.emptyList());
  ...
  class SendMessageTests {
    ...
    void sendMessage_validUser_callsChatService() {
      ...
      // SỬA DÒNG NÀY:
      // Cũ: doNothing().when(chatService).sendMessage(eq(mockAuthentication),
      // eq(chatMessageRequest));
      // Mới:
    ...
    @Test
    @DisplayName(
        "sendMessage - User Not Properly Authenticated in Header - Throws AccessDeniedException (caught by handler)")
    void sendMessage_userNotAuthenticatedInHeader_throwsAccessDenied() {
      headerAccessor.setUser(
          mock(Principal.class)); // Giả lập Principal không phải là Authentication
  ...
  class MessageExceptionHandlerTests {
    ...
    void handleException_genericException() {
      ...
      assertEquals(
          ...
          errorEvent.getMessage()); // Message được che giấu
    }
  ...
  class GetAuthenticationHelperTests {
    ...
    @Test
    @DisplayName(
        "getAuthentication - Principal is Not Authentication - Throws AccessDeniedException")
    void getAuthentication_principalIsNotAuthentication_throwsAccessDeniedException() {
      headerAccessor.setUser(
          mock(Principal.class)); // Một Principal bất kỳ không phải Authentication
      assertThrows(
          AccessDeniedException.class,
