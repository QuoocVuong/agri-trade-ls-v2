...
class WebSocketEventListenerTest {
    ...
    void setUp() {
        ...
        mockAuth1 = new UsernamePasswordAuthenticationToken(testUser1.getEmail(), null, Collections.emptyList());
        mockAuth2 = new UsernamePasswordAuthenticationToken(testUser2.getEmail(), null, Collections.emptyList());
        ...
        org.springframework.test.util.ReflectionTestUtils.setField(webSocketEventListener, "userSessions", userSessionsMap);
        org.springframework.test.util.ReflectionTestUtils.setField(webSocketEventListener, "onlineUsersStatus", onlineUsersStatusMap);


        lenient().when(userRepository.findByEmail(testUser1.getEmail())).thenReturn(Optional.of(testUser1));
        lenient().when(userRepository.findByEmail(testUser2.getEmail())).thenReturn(Optional.of(testUser2));
    ...
    private SessionDisconnectEvent createDisconnectEvent(Principal principal, String sessionId, String closeStatus) {
        SimpMessageHeaderAccessor headerAccessor = SimpMessageHeaderAccessor.create();
    ...
    class ConnectListenerTests {
        ...
        @Test
        @DisplayName("Handle Connect - Existing User, New Session - Should Not Broadcast Again If Already Online")
        void handleWebSocketConnectListener_existingUserNewSession_shouldNotBroadcastIfAlreadyOnline() {
            ...
            webSocketEventListener.handleWebSocketConnectListener(createConnectEvent(mockAuth1, SESSION_ID_1));
            reset(messagingTemplate); // Reset mock để chỉ kiểm tra lần gọi thứ hai

            // User 1 connects second session
            webSocketEventListener.handleWebSocketConnectListener(createConnectEvent(mockAuth1, SESSION_ID_2));

            assertEquals(2, userSessionsMap.get(testUser1.getId()).size());
            assertTrue(onlineUsersStatusMap.getOrDefault(testUser1.getId(), false));
            verify(messagingTemplate, never()).convertAndSend(eq("/topic/presence"), any(Map.class)); // Không broadcast lại
        }
    ...
    class DisconnectListenerTests {
        ...
        @Test
        @DisplayName("Handle Disconnect - User Has Other Sessions - Should Not Mark Offline or Broadcast")
        ...
        void handleWebSocketDisconnectListener_sessionIdNotFound_shouldLogWarn() {
            SessionDisconnectEvent disconnectEvent = createDisconnectEvent(null, "unknown_session", "1000");
            webSocketEventListener.handleWebSocketDisconnectListener(disconnectEvent);
            verify(messagingTemplate, never()).convertAndSend(anyString(), any(Map.class));
        }
    }
    ...
    class HelperMethodTests {
        ...
        void isUserOnline_returnsCorrectStatus() {
            ...
            webSocketEventListener.handleWebSocketDisconnectListener(createDisconnectEvent(null, SESSION_ID_1, "1000"));
            assertFalse(webSocketEventListener.isUserOnline(testUser1.getId()));
        }
        ...
        void getOnlineUserIds_returnsCorrectSet() {
            ...
            webSocketEventListener.handleWebSocketConnectListener(createConnectEvent(mockAuth1, SESSION_ID_1));
            webSocketEventListener.handleWebSocketConnectListener(createConnectEvent(mockAuth2, SESSION_ID_3)); // User 2 connects
