...
class ReviewServiceImplTest {
  ...
  void setUp() {
    ...
    lenient()
        .when(userRepository.findByEmail(testConsumer.getEmail()))
        .thenReturn(Optional.of(testConsumer));
    lenient()
  ...
  class CreateReviewTests {
    ...
    void createReview_success() {
      ...
      when(orderRepository.existsByIdAndBuyerIdAndOrderItemsProductId(
          ...
          .thenReturn(true); // User đã mua
      when(reviewRepository.existsByConsumerIdAndProductIdAndOrderId(
              testConsumer.getId(), testProduct.getId(), testOrder.getId()))
          .thenReturn(false); // Chưa review cho order này
      when(reviewRepository.existsByConsumerIdAndProductId(
              testConsumer.getId(), testProduct.getId()))
          .thenReturn(false); // Chưa review sản phẩm này bao giờ
      when(orderRepository.findById(testOrder.getId())).thenReturn(Optional.of(testOrder));
      when(reviewMapper.requestToReview(reviewRequest))
          .thenReturn(testReview); // Giả sử mapper trả về testReview (chưa có ID)
      ...
      when(productRepository.findById(testProduct.getId()))
          .thenReturn(
              Optional.of(testProduct)); // Cần cho findById trong updateProductAverageRating
    ...
    void createReview_userHasNotPurchased_throwsBadRequest() {
      ...
      when(orderRepository.existsByIdAndBuyerIdAndOrderItemsProductId(
              testOrder.getId(), testConsumer.getId(), testProduct.getId()))
          .thenReturn(false); // User chưa mua
  ...
  class GetReviewTests {
    ...
    void getReviewsForFarmerProducts_success() {
      ...
      Page<Review> reviewPage =
          new PageImpl<>(List.of(testReview), pageable, 1); // testReview có product của testFarmer
  ...
  class AdminReviewManagementTests {
    ...
    void approveReview_success() {
      ...
      when(reviewRepository.getAverageRatingAndCountByProductIdAndStatus(
              testProduct.getId(), ReviewStatus.APPROVED))
          .thenReturn(
              Optional.of(new Object[] {4.0, 2L})); // Giả sử sau khi duyệt, rating là 4, count là 2
      when(productRepository.save(any(Product.class))).thenReturn(testProduct);
