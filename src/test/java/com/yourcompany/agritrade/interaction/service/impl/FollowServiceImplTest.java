...
class FollowServiceImplTest {
    ...
    class FollowUserTests {
        ...
        void followUser_success() {
            ...
            when(userFollowRepository.existsByFollowerIdAndFollowingId(currentUser.getId(), userToFollow.getId())).thenReturn(false);
            when(userFollowRepository.save(any(UserFollow.class))).thenAnswer(inv -> inv.getArgument(0));
            // Mock cho updateFollowCounts
            when(userRepository.findById(currentUser.getId())).thenReturn(Optional.of(currentUser)); // Cho follower
            when(userRepository.findById(userToFollow.getId())).thenReturn(Optional.of(userToFollow)); // Cho following
            when(userFollowRepository.countByFollowerId(currentUser.getId())).thenReturn(1L); // Sau khi follow
            when(userFollowRepository.countByFollowingId(userToFollow.getId())).thenReturn(1L); // Sau khi được follow
            ...
            assertTrue(savedUsers.stream().anyMatch(u -> u.getId().equals(currentUser.getId()) && u.getFollowingCount() == 1));
            assertTrue(savedUsers.stream().anyMatch(u -> u.getId().equals(userToFollow.getId()) && u.getFollowerCount() == 1));
        ...
        void followUser_alreadyFollowing_shouldDoNothing() {
            when(userRepository.findById(userToFollow.getId())).thenReturn(Optional.of(userToFollow));
            when(userFollowRepository.existsByFollowerIdAndFollowingId(currentUser.getId(), userToFollow.getId())).thenReturn(true);
        ...
        void followUser_userToFollowNotFound_throwsResourceNotFoundException() {
            ...
            assertThrows(ResourceNotFoundException.class,
                    () -> followService.followUser(authentication, 99L));
        }
    }
    ...
    class UnfollowUserTests {
        ...
        void unfollowUser_success() {
            when(userFollowRepository.existsByFollowerIdAndFollowingId(currentUser.getId(), userToFollow.getId())).thenReturn(true);
            doNothing().when(userFollowRepository).deleteByFollowerIdAndFollowingId(currentUser.getId(), userToFollow.getId());
            // Mock cho updateFollowCounts
            when(userRepository.findById(currentUser.getId())).thenReturn(Optional.of(currentUser));
            when(userRepository.findById(userToFollow.getId())).thenReturn(Optional.of(userToFollow));
            when(userFollowRepository.countByFollowerId(currentUser.getId())).thenReturn(0L); // Sau khi unfollow
            when(userFollowRepository.countByFollowingId(userToFollow.getId())).thenReturn(0L); // Sau khi bị unfollow
            when(userRepository.save(any(User.class))).thenAnswer(inv -> inv.getArgument(0));

            followService.unfollowUser(authentication, userToFollow.getId());

            verify(userFollowRepository).deleteByFollowerIdAndFollowingId(currentUser.getId(), userToFollow.getId());
            ArgumentCaptor<User> userCaptor = ArgumentCaptor.forClass(User.class);
            verify(userRepository, times(2)).save(userCaptor.capture());
            List<User> savedUsers = userCaptor.getAllValues();
            assertTrue(savedUsers.stream().anyMatch(u -> u.getId().equals(currentUser.getId()) && u.getFollowingCount() == 0));
            assertTrue(savedUsers.stream().anyMatch(u -> u.getId().equals(userToFollow.getId()) && u.getFollowerCount() == 0));
        }
    ...
    class GetFollowsTests {
        ...
        void getMyFollowing_success() {
            ...
            FollowUserResponse response1 = new FollowUserResponse(); response1.setUserId(userToFollow.getId());
            FollowUserResponse response2 = new FollowUserResponse(); response2.setUserId(anotherUser.getId());
            List<FollowUserResponse> expectedResponseList = List.of(response1, response2);

            when(userFollowRepository.findFollowingByFollowerId(currentUser.getId())).thenReturn(followingUsersList);
            when(userFollowRepository.countByFollowerId(currentUser.getId())).thenReturn((long) followingUsersList.size());
            when(followUserMapper.toFollowUserResponseList(followingUsersList)).thenReturn(expectedResponseList);
        ...
        void getMyFollowers_success() {
            Pageable pageable = PageRequest.of(0, 10);
            List<User> followerUsersList = List.of(anotherUser); // Giả sử anotherUser đang follow currentUser
            FollowUserResponse response1 = new FollowUserResponse(); response1.setUserId(anotherUser.getId());
            List<FollowUserResponse> expectedResponseList = List.of(response1);

            when(userFollowRepository.findFollowersByFollowingId(currentUser.getId())).thenReturn(followerUsersList);
            when(userFollowRepository.countByFollowingId(currentUser.getId())).thenReturn((long) followerUsersList.size());
            when(followUserMapper.toFollowUserResponseList(followerUsersList)).thenReturn(expectedResponseList);
        ...
        void getFollowersPublic_userExists_success() {
            Pageable pageable = PageRequest.of(0, 10);
            List<User> followerUsersList = List.of(currentUser); // Giả sử currentUser đang follow userToFollow
            FollowUserResponse response1 = new FollowUserResponse(); response1.setUserId(currentUser.getId());
            List<FollowUserResponse> expectedResponseList = List.of(response1);

            when(userRepository.existsById(userToFollow.getId())).thenReturn(true);
            when(userFollowRepository.findFollowersByFollowingId(userToFollow.getId())).thenReturn(followerUsersList);
            when(userFollowRepository.countByFollowingId(userToFollow.getId())).thenReturn((long) followerUsersList.size());
            when(followUserMapper.toFollowUserResponseList(followerUsersList)).thenReturn(expectedResponseList);

            Page<FollowUserResponse> result = followService.getFollowersPublic(userToFollow.getId(), pageable);
        ...
        void getFollowersPublic_userNotExists_throwsResourceNotFoundException() {
            ...
            assertThrows(ResourceNotFoundException.class,
                    () -> followService.getFollowersPublic(99L, pageable));
        }
    }
    ...
    class IsFollowingTest {
        ...
        void isFollowing_whenNotFollowing_returnsFalse() {
            when(userFollowRepository.existsByFollowerIdAndFollowingId(currentUser.getId(), anotherUser.getId())).thenReturn(false);
