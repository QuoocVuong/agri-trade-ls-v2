...
class FarmerReviewControllerTest {
  ...
  void getMyProductReviews_success() throws Exception {
    ...
    mockMvc
        .perform(get("/api/farmer/reviews").param("page", "0").param("size", "10"))
    ...
    verify(reviewService)
        .getReviewsForFarmerProducts(any(Authentication.class), any(Pageable.class));
  }

  @Test
  @DisplayName(
      "GET /api/farmer/reviews - Lấy Đánh giá Sản phẩm của Tôi (Farmer) - Không có đánh giá")
  void getMyProductReviews_emptyList() throws Exception {
    ...
    mockMvc
        .perform(get("/api/farmer/reviews"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success", is(true)))
