...
class FollowControllerTest {
  ...
  class FollowUnfollowTests {
    ...
    void followUser_success() throws Exception {
      ...
      mockMvc
          .perform(post("/api/follows/following/{followingId}", followingId))
    ...
    void followUser_userNotFound_throwsNotFound() throws Exception {
      ...
      doThrow(new ResourceNotFoundException("User", "id", followingId))
          .when(followService)
          .followUser(any(Authentication.class), eq(followingId));
    ...
    void followUser_cannotFollowSelf_throwsBadRequest() throws Exception {
      ...
      doThrow(new BadRequestException("You cannot follow yourself."))
          .when(followService)
          .followUser(any(Authentication.class), eq(selfId));

      mockMvc
          .perform(post("/api/follows/following/{followingId}", selfId))
    ...
    void unfollowUser_success() throws Exception {
      ...
      mockMvc
          .perform(delete("/api/follows/following/{followingId}", followingId))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)))
  ...
  class GetFollowListsTests {
    ...
    @WithMockUser
    @DisplayName(
        "GET /api/follows/following/my - Lấy Danh sách Người Tôi Đang Theo dõi - Thành công")
    void getMyFollowing_success() throws Exception {
      ...
      mockMvc
          .perform(get("/api/follows/following/my").param("page", "0").param("size", "20"))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)))
          .andExpect(jsonPath("$.data.content", hasSize(1)))
          .andExpect(
              jsonPath("$.data.content[0].userId", is(followUserResponse.getUserId().intValue())));
    }
    ...
    void getMyFollowers_success() throws Exception {
      ...
      mockMvc
          .perform(get("/api/follows/followers/my").param("page", "0").param("size", "20"))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)))
          .andExpect(jsonPath("$.data.content", hasSize(1)))
          .andExpect(
    ...
    @Test
    @DisplayName(
        "GET /api/follows/followers/user/{userId} - Lấy Danh sách Người Theo dõi của User Khác (Public) - Thành công")
    void getFollowersPublic_success() throws Exception {
      ...
      mockMvc
          .perform(
              get("/api/follows/followers/user/{userId}", userId)
                  .param("page", "0")
                  .param("size", "20"))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)))
    ...
    void getFollowersPublic_userNotFound_throwsNotFound() throws Exception {
      ...
      mockMvc
          .perform(get("/api/follows/followers/user/{userId}", userId))
          .andExpect(status().isNotFound())
          .andExpect(jsonPath("$.success", is(false)))
          .andExpect(jsonPath("$.message", is("User not found with id : '99'")));
    }
  ...
  class CheckFollowingStatusTests {
    ...
    void checkFollowingStatus_isFollowing_returnsTrue() throws Exception {
      ...
      mockMvc
          .perform(get("/api/follows/following/status/{followingId}", followingId))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.success", is(true)))
