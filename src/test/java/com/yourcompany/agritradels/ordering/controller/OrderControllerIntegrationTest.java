package com.yourcompany.agritradels.ordering.controller;

import com.fasterxml.jackson.core.type.TypeReference; // Import TypeReference
import com.fasterxml.jackson.databind.ObjectMapper;
import com.yourcompany.agritradels.AbstractIntegrationTest;
...
import com.yourcompany.agritradels.common.dto.ApiResponse; // Import ApiResponse
...
import com.yourcompany.agritradels.ordering.dto.response.OrderResponse; // Import OrderResponse
import com.yourcompany.agritradels.ordering.dto.response.OrderSummaryResponse; // Import OrderSummaryResponse
import com.yourcompany.agritradels.ordering.repository.CartItemRepository;
import com.yourcompany.agritradels.ordering.repository.OrderItemRepository;
import com.yourcompany.agritradels.ordering.repository.OrderRepository;
import com.yourcompany.agritradels.ordering.repository.PaymentRepository;
...
import com.yourcompany.agritradels.usermanagement.repository.AddressRepository;
...
class OrderControllerIntegrationTest extends AbstractIntegrationTest {
    ...
    void setUpTestData() {
        // Tạo Roles
        buyerRole = roleRepository.findByName(RoleType.ROLE_CONSUMER).orElseGet(() -> roleRepository.save(new Role(RoleType.ROLE_CONSUMER)));
        farmerRole = roleRepository.findByName(RoleType.ROLE_FARMER).orElseGet(() -> roleRepository.save(new Role(RoleType.ROLE_FARMER)));
        adminRole = roleRepository.findByName(RoleType.ROLE_ADMIN).orElseGet(() -> roleRepository.save(new Role(RoleType.ROLE_ADMIN)));
    ...
    @WithMockUser(username = "buyer.test@example.com", roles = {"CONSUMER"})
    ...
    void getMyOrdersAsBuyer_Success() throws Exception {
        ...
        User otherBuyer = userRepository.save(User.builder().email("other@test.com").passwordHash("...").fullName("Other").isActive(true).roles(Collections.singleton(buyerRole)).build());
        ...
        MvcResult result = mockMvc.perform(get("/api/orders/my")
                ...
                .andExpect(jsonPath("$.success", is(true)))
                .andExpect(jsonPath("$.data.content", hasSize(2))) // Chỉ mong đợi 2 đơn hàng của testBuyer
        ...
        // assertThat(response.getData().getContent()).allMatch(o -> o.getBuyerName().equals(testBuyer.getFullName()));
    }
    ...
    void getMyOrderDetailsById_Success_BuyerIsOwner() throws Exception {
        Order order = createTestOrder(testBuyer, testFarmer, OrderStatus.PROCESSING, PaymentStatus.PENDING);
    ...
    void getMyOrderDetailsById_Fail_BuyerIsNotOwner() throws Exception {
        User otherBuyer = userRepository.save(User.builder().email("other2@test.com").passwordHash("...").fullName("Other 2").isActive(true).roles(Collections.singleton(buyerRole)).build());
        Order otherOrder = createTestOrder(otherBuyer, testFarmer, OrderStatus.PENDING, PaymentStatus.PENDING);
    ...
    @Test
    @DisplayName("[POST /api/orders/{orderId}/cancel] Buyer hủy đơn hàng thành công khi trạng thái hợp lệ")
    @WithMockUser(username = "buyer.test@example.com", roles = {"CONSUMER"})
    void cancelMyOrder_Success_WhenStatusIsCancellable() throws Exception {
        Order order = createTestOrder(testBuyer, testFarmer, OrderStatus.CONFIRMED, PaymentStatus.PENDING); // CONFIRMED vẫn hủy được
    ...
    @Test
    @DisplayName("[POST /api/orders/{orderId}/cancel] Buyer hủy đơn hàng thất bại khi trạng thái không hợp lệ")
    @WithMockUser(username = "buyer.test@example.com", roles = {"CONSUMER"})
    void cancelMyOrder_Fail_WhenStatusIsNotCancellable() throws Exception {
        Order order = createTestOrder(testBuyer, testFarmer, OrderStatus.SHIPPING, PaymentStatus.PENDING); // SHIPPING không hủy được
    ...
    private Order createTestOrder(User buyer, User farmer, OrderStatus initialStatus, PaymentStatus paymentStatus) {
