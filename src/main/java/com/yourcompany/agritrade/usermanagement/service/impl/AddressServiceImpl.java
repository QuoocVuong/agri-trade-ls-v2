...
public class AddressServiceImpl implements AddressService {
    ...
    public AddressResponse addMyAddress(Authentication authentication, AddressRequest request) {
        ...
        log.info("Added new address with ID: {} for user ID: {}", savedAddress.getId(), currentUser.getId());
        return addressMapper.toAddressResponse(savedAddress);
    }

    @Override
    @Transactional
    public AddressResponse updateMyAddress(Authentication authentication, Long addressId, AddressRequest request) {
        User currentUser = getUserFromAuthentication(authentication);
        Address existingAddress = addressRepository.findByIdAndUserId(addressId, currentUser.getId())
                .orElseThrow(() -> new ResourceNotFoundException("Address", "id", addressId));
        ...
        } else if (request.getIsDefault() != null && !request.getIsDefault() && existingAddress.isDefault()) {
        ...
        log.info("Updated address with ID: {} for user ID: {}", updatedAddress.getId(), currentUser.getId());
    ...
    public void deleteMyAddress(Authentication authentication, Long addressId) {
        ...
        if (address.isDefault()) {
            List<Address> otherAddresses = addressRepository.findByUserId(currentUser.getId()).stream()
                    .filter(a -> !a.getId().equals(addressId)).toList();
            if (!otherAddresses.isEmpty()) {
                // Đặt địa chỉ khác làm mặc định (ví dụ: cái mới nhất)
                Address newDefault = otherAddresses.stream().max(java.util.Comparator.comparing(Address::getCreatedAt)).get();
    ...
    public void setMyDefaultAddress(Authentication authentication, Long addressId) {
        User currentUser = getUserFromAuthentication(authentication);
        Address newDefaultAddress = addressRepository.findByIdAndUserId(addressId, currentUser.getId())
                .orElseThrow(() -> new ResourceNotFoundException("Address", "id", addressId));
    ...
    private void unsetDefaultForOtherAddresses(Long userId, Long excludeAddressId) {
        List<Address> currentDefaults = addressRepository.findByUserId(userId).stream()
                .filter(Address::isDefault)
                .filter(a -> excludeAddressId == null || !a.getId().equals(excludeAddressId)) // Loại trừ địa chỉ đang được set/update
                .toList();
