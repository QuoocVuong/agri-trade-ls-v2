...
import com.yourcompany.agritrade.common.model.ReviewStatus; // Import ReviewStatus
...
import com.yourcompany.agritrade.ordering.mapper.OrderMapper; // Import OrderMapper
...
public class DashboardServiceImpl implements DashboardService {
    ...
    @Override
    public List<FarmerChartDataResponse> getFarmerOrderCountChartData(Authentication authentication, LocalDate startDate, LocalDate endDate) {
        User farmer = getUserFromAuthentication(authentication);
        Long farmerId = farmer.getId();
        ...
        List<Tuple> dailyCounts = orderRepository.countOrdersByDayForFarmer(farmerId, startDateTime, endDateTime);
    ...
    @Override
    public List<FarmerChartDataResponse> getFarmerRevenueChartData(Authentication authentication, LocalDate startDate, LocalDate endDate) {
        User farmer = getUserFromAuthentication(authentication);
        Long farmerId = farmer.getId();
        ...
        List<Tuple> dailyRevenues = orderRepository.sumRevenueByDayForFarmer(
                farmerId,
                startDateTime,
                endDateTime,
...
//    }
    ...
    public List<TimeSeriesDataPoint<BigDecimal>> getDailyRevenueForAdminChart(LocalDate startDate, LocalDate endDate) {
        ...
        // Tạo Map từ kết quả query (LocalDate -> BigDecimal revenue)
        Map<LocalDate, BigDecimal> revenuesMap = results.stream()
                .filter(result -> result != null && result.length > 1 && result[0] != null && result[1] != null) // Kiểm tra null kỹ hơn
                .collect(Collectors.toMap(
        ...
        // Tạo kết quả cuối cùng, điền 0 cho những ngày không có doanh thu
        return dateRange.stream()
                .map(date -> new TimeSeriesDataPoint<>(date, revenuesMap.getOrDefault(date, BigDecimal.ZERO)))
                .collect(Collectors.toList());
    }
...
//    }
    ...
    private LocalDate parseDateFromResult(Object dateObject) {
        ...
        log.warn("Unexpected date type returned from query: {}", dateObject != null ? dateObject.getClass().getName() : "null");
