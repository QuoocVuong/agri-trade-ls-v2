...
public class RoleServiceImpl implements RoleService {
  ...
  public RoleResponse updateRolePermissions(Integer id, RoleUpdateRequest request) {
    ...
    Set<Permission> newPermissions =
        permissionRepository.findByNameIn(request.getPermissionNames());
    ...
    if (newPermissions.size() != request.getPermissionNames().size()) {
      Set<String> foundNames =
          newPermissions.stream().map(Permission::getName).collect(Collectors.toSet());
      request.getPermissionNames().removeAll(foundNames); // Tìm ra tên không hợp lệ

      Set<String> invalidNames =
          new HashSet<>(request.getPermissionNames()); // Tạo bản sao để không sửa request gốc
      invalidNames.removeAll(foundNames); // Giữ lại những tên không tìm thấy
      throw new ResourceNotFoundException(
          "Permission", "names", invalidNames.toString()); // Gọi constructor 3 tham số
    }
