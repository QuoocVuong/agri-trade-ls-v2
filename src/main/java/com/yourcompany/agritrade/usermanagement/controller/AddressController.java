...
public class AddressController {
    ...
    @GetMapping("/my/{id}")
    public ResponseEntity<ApiResponse<AddressResponse>> getMyAddressById(Authentication authentication, @PathVariable Long id) {
        AddressResponse address = addressService.getMyAddressById(authentication, id);
        return ResponseEntity.ok(ApiResponse.success(address));
    }

    @PostMapping("my")
    public ResponseEntity<ApiResponse<AddressResponse>> addMyAddress(Authentication authentication,
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success(newAddress, "Address added successfully"));
    }

    @PutMapping("/my/{id}")
    public ResponseEntity<ApiResponse<AddressResponse>> updateMyAddress(Authentication authentication,
    ...
    @DeleteMapping("/my/{id}")
    public ResponseEntity<ApiResponse<Void>> deleteMyAddress(Authentication authentication, @PathVariable Long id) {
        addressService.deleteMyAddress(authentication, id);
        return ResponseEntity.ok(ApiResponse.success("Address deleted successfully")); // Hoặc trả về 204 No Content
    }

    @PutMapping("/my/{id}/default")
    public ResponseEntity<ApiResponse<Void>> setMyDefaultAddress(Authentication authentication, @PathVariable Long id) {
