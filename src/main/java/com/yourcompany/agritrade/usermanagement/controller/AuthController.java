...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<LoginResponse>> refreshToken(@RequestBody String refreshToken) { // Nhận refresh token từ body
    ...
    public ResponseEntity<ApiResponse<Void>> logoutUser(HttpServletRequest request) {
        ...
        if (StringUtils.hasText(jwt)) {
            ...
            if (jti != null && expiryDate != null) {
                ...
                log.info("User {} logged out. Token JTI {} added to blacklist.", SecurityContextHolder.getContext().getAuthentication().getName(), jti);
            } else {
                log.warn("Could not add token to blacklist: JTI or ExpiryDate is null for the provided token.");
            }
        } else {
