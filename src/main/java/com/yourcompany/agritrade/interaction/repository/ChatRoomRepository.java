...
public interface ChatRoomRepository extends JpaRepository<ChatRoom, Long> {
   ...
   */
  @Query(
      "SELECT cr FROM ChatRoom cr WHERE (cr.user1.id = :userId1 AND cr.user2.id = :userId2) OR (cr.user1.id = :userId2 AND cr.user2.id = :userId1)")
  Optional<ChatRoom> findRoomBetweenUsers(
      @Param("userId1") Long userId1, @Param("userId2") Long userId2);

  /**
   * Tìm tất cả các phòng chat mà một người dùng tham gia. Sắp xếp theo thời gian tin nhắn cuối cùng
   * giảm dần để hiển thị gần đây nhất lên đầu. Sử dụng JOIN FETCH để tải thông tin user và tin nhắn
   * cuối cùng (nếu cần).
   *
  ...
  @Query(
      "SELECT cr FROM ChatRoom cr "
          ...
          + // Fetch thông tin user1
          "LEFT JOIN FETCH cr.user2 u2 "
          + // Fetch thông tin user2
          "LEFT JOIN FETCH cr.lastMessage lm "
          + // Fetch tin nhắn cuối
          "LEFT JOIN FETCH lm.sender "
          + // Fetch người gửi tin nhắn cuối
          "WHERE cr.user1.id = :userId OR cr.user2.id = :userId "
          + "ORDER BY cr.lastMessageTime DESC NULLS LAST, cr.updatedAt DESC") // Ưu tiên phòng có
  // tin nhắn mới, phòng
  // chưa có tin nhắn
  // xuống cuối
  ...
  @Query(
      "SELECT SUM(CASE WHEN cr.user1.id = :userId THEN cr.user1UnreadCount ELSE cr.user2UnreadCount END) "
