...
public class ExcelExportService {
    ...
    public ByteArrayInputStream ordersToExcel(List<OrderSummaryResponse> orders) throws IOException {
        String[] COLUMNS = {"Mã ĐH", "Ngày đặt", "Người mua", "Người bán", "Tổng tiền", "Trạng thái ĐH", "Trạng thái TT"};
        ...
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            for (OrderSummaryResponse order : orders) {
                ...
                row.createCell(1).setCellValue(order.getCreatedAt().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")));
                ...
                row.createCell(5).setCellValue(order.getStatus() != null ? order.getStatus().name() : "N/A");
                row.createCell(6).setCellValue(order.getPaymentStatus() != null ? order.getPaymentStatus().name() : "N/A");
            }
