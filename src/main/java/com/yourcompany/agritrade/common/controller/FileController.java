...
public class FileController {
    ...
    public ResponseEntity<ApiResponse<FileUploadResponse>> uploadFile(
        ...
        FileUploadResponse responseData = new FileUploadResponse(blobPath, fileDownloadUri, file.getContentType(), file.getSize());
...
//                    String fileDownloadUri = fileStorageService.getFileUrl(filename, type);
                    String blobPath = fileStorageService.store(file, type);
                    String fileDownloadUri = fileStorageService.getFileUrl(blobPath); // <<< GỌI VỚI 1 THAM SỐ
                    return new FileUploadResponse(blobPath, fileDownloadUri, file.getContentType(), file.getSize());
                })
    ...
    public ResponseEntity<Resource> downloadFile(HttpServletRequest request) {
        ...
        if (!StringUtils.hasText(finalBlobPath)) {
            log.warn("Could not extract blobPath from request URI for download: {}", request.getRequestURI());
        ...
        return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "inline; filename=\"" + (filename != null ? filename : "download") + "\"")
                .body(resource);
    }
    ...
    public ResponseEntity<ApiResponse<Void>> deleteFile(HttpServletRequest request) {
        ...
        String finalBlobPath = extractBlobPath(request, ""); // Giả sử không có prefix cố định cho delete
        // hoặc nếu endpoint là /delete/** thì truyền "/delete/"
        if (!StringUtils.hasText(finalBlobPath)) {
            return ResponseEntity.badRequest().body(ApiResponse.badRequest("Invalid file path for deletion"));
        }
        try {
    ...
    // ****** THÊM HÀM HELPER NÀY ******
    /**
     * Trích xuất phần đường dẫn file (blobPath) từ HttpServletRequest.
     * Nó loại bỏ phần base path của API và prefix của endpoint (nếu có).
     * @param request HttpServletRequest hiện tại.
     * @param endpointPrefix Prefix của endpoint cần loại bỏ (ví dụ: "/download/", "/delete/", hoặc "" nếu không có).
    ...
    private String extractBlobPath(HttpServletRequest request, String endpointPrefix) {
        ...
        if (path == null || bestMatchPattern == null) {
            ...
            if (startIndex != -1) {
                String remainingPath = requestUri.substring(startIndex + (contextPath + servletPath).length());
        ...
        // Ví dụ: nếu bestMatchPattern là "/api/files/download/**", extractedPath sẽ là "subfolder/file.jpg"
        ...
        if (StringUtils.hasText(endpointPrefix) && extractedPath.startsWith(endpointPrefix.startsWith("/") ? endpointPrefix.substring(1) : endpointPrefix)) {
            return extractedPath.substring((endpointPrefix.startsWith("/") ? endpointPrefix.substring(1) : endpointPrefix).length());
        }
