...
import org.springframework.security.authentication.UsernamePasswordAuthenticationToken; // Ví dụ
import org.springframework.stereotype.Component;
// Import các lớp xử lý JWT của bạn
import com.yourcompany.agritrade.config.security.JwtTokenProvider;
//import com.yourcompany.agritradels.usermanagement.service.UserDetailsServiceImpl; // Hoặc service tương ứng
...
public class AuthChannelInterceptor implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if (accessor != null && StompCommand.CONNECT.equals(accessor.getCommand())) {
            ...
            log.debug("STOMP DISCONNECT received for user: {}", (user != null ? user.getName() : "unknown"));
        } else if (accessor != null && StompCommand.SUBSCRIBE.equals(accessor.getCommand())) {
            Principal user = accessor.getUser();
            log.debug("STOMP SUBSCRIBE received for user: {} to destination: {}", (user != null ? user.getName() : "anonymous"), accessor.getDestination());
            // Kiểm tra quyền subscribe nếu cần
        } else if (accessor != null && StompCommand.SEND.equals(accessor.getCommand())) {
            Principal user = accessor.getUser();
            log.debug("STOMP SEND received for user: {} to destination: {}", (user != null ? user.getName() : "anonymous"), accessor.getDestination());
