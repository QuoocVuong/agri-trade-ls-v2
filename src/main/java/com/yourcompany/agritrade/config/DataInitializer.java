...
import com.yourcompany.agritrade.usermanagement.domain.Permission; // Import
import com.yourcompany.agritrade.usermanagement.domain.Role;
import com.yourcompany.agritrade.usermanagement.domain.User;
import com.yourcompany.agritrade.usermanagement.repository.PermissionRepository; // Import
...
public class DataInitializer implements CommandLineRunner {
  ...
  private static final List<PermissionData> ALL_PERMISSIONS = Arrays.asList(
          ...
          new PermissionData("PRODUCT_MANAGE_OWN", "Quyền quản lý sản phẩm của chính mình (Farmer)"),
  ...
  private static final Map<RoleType, List<String>> ROLE_PERMISSION_MAPPING = Map.of(
          RoleType.ROLE_ADMIN, List.of(
                  "USER_READ_ALL", "USER_READ_DETAIL", "USER_UPDATE_STATUS", "USER_UPDATE_ROLES",
                  "USER_MANAGE_PROFILES", "PRODUCT_MANAGE_ALL", "PRODUCT_READ", "ORDER_READ_ALL",
                  "ORDER_UPDATE_STATUS_ALL", "PERMISSION_MANAGE" // Admin có quyền quản lý permission
          ),
          RoleType.ROLE_FARMER, List.of(
                  "PRODUCT_MANAGE_OWN", "ORDER_READ_OWN", "ORDER_UPDATE_STATUS_OWN", "PRODUCT_READ"
          ),
          RoleType.ROLE_CONSUMER, List.of(
                  "ORDER_CREATE", "ORDER_READ_OWN", "PRODUCT_READ"
          ),
          RoleType.ROLE_BUSINESS_BUYER, List.of(
                  "ORDER_CREATE", "ORDER_READ_OWN", "PRODUCT_READ"
          )
  );
  ...
  private void initializeRoleAndPermissions(RoleType roleType) {
    ...
    if (expectedPermissionNames != null && !expectedPermissionNames.isEmpty()) {
      ...
      List<String> permissionsToAssignNames = expectedPermissionNames.stream()
              .filter(name -> !currentPermissionNames.contains(name))
              .collect(Collectors.toList());

      if (!permissionsToAssignNames.isEmpty()) {
        Set<Permission> permissionsToAssign = permissionRepository.findByNameIn(new HashSet<>(permissionsToAssignNames));
        if (permissionsToAssign.size() != permissionsToAssignNames.size()) {
          log.warn("Could not find all expected permissions for role {}: Missing {}", roleType,
                  permissionsToAssignNames.stream().filter(name -> permissionsToAssign.stream().noneMatch(p -> p.getName().equals(name))).collect(Collectors.toList()));
        }
        currentPermissions.addAll(permissionsToAssign);
        role.setPermissions(currentPermissions);
        roleRepository.save(role);
        log.info("Assigned/Updated permissions for role {}: {}", roleType, permissionsToAssign.stream().map(Permission::getName).collect(Collectors.toList()));
      }
    }
  }
