...
public class TokenBlacklistService {
    ...
    /**
     * Thêm JTI của token vào blacklist.
     * Token sẽ tự động bị xóa khỏi blacklist khi nó hết hạn.
    ...
    public void addToBlacklist(String jti, Date expiryDate) {
        ...
        long ttlMillis = expiryDate.getTime() - now; // Thời gian còn lại của token (tính bằng mili giây)

        if (ttlMillis > 0) {
            try {
                // Key trong Redis sẽ là "jwt_blacklist:jti_value"
                // Value có thể là bất cứ gì, ví dụ "blacklisted" hoặc thời gian blacklist
                redisTemplate.opsForValue().set(BLACKLIST_PREFIX + jti, "blacklisted", ttlMillis, TimeUnit.MILLISECONDS);
                ...
                // Cân nhắc xử lý lỗi này, ví dụ: nếu Redis không hoạt động, logout có thể không hiệu quả 100%
            }
        } else {
            log.warn("Attempted to add JTI {} to blacklist, but its expiry date is in the past. Skipping.", jti);
        }
    }
