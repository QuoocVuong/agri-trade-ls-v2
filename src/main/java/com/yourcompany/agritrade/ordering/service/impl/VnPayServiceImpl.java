...
public class VnPayServiceImpl implements PaymentGatewayService {
    ...
    @Value("${payment.vnpay.version}") private String vnpVersion;
    @Value("${payment.vnpay.tmnCode}") private String vnpTmnCode;
    @Value("${payment.vnpay.hashSecret}") private String vnpHashSecret;
    @Value("${payment.vnpay.url}") private String vnpPayUrl;
    @Value("${app.backend.vnpayIpnUrl}") private String vnpIpnUrl; // URL Backend nhận IPN
    ...
    public PaymentUrlResponse createVnPayPaymentUrl(Order order, String ipAddress, String clientReturnUrl) {
        ...
        // vnp_Params.put("vnp_BankCode", ""); // Bỏ trống để hiển thị cổng chọn ngân hàng, hoặc truyền mã cụ thể
