...
public interface OrderRepository extends JpaRepository<Order, Long>, JpaSpecificationExecutor<Order> { // Thêm JpaSpecificationExecutor
    ...
    @Query("SELECT o FROM Order o " +
            ...
            "LEFT JOIN FETCH f.farmerProfile fp " + // Fetch farmer profile để có bank info nếu chuyển cho farmer
            "LEFT JOIN FETCH o.orderItems oi " +
            "LEFT JOIN FETCH oi.product p " +
            "LEFT JOIN FETCH p.images " + // << QUAN TRỌNG: FETCH IMAGES
...
//    Optional<Order> findByOrderCode(String orderCode); // Spring Data JPA tự tạo query
    ...
    @Query("SELECT CASE WHEN COUNT(o.id) > 0 THEN true ELSE false END " +
            ...
            "AND o.status = com.yourcompany.agritrade.ordering.domain.OrderStatus.DELIVERED") // Chỉ tính đơn đã giao
    ...
    /**
     * Query đếm số đơn hàng theo ngày cho farmer cụ thể.
     * Trả về Tuple với cột 0 là LocalDate (orderDate), cột 1 là Long (orderCount).
     */
    @Query("""
           ...
           """)
    List<Tuple> countOrdersByDayForFarmer(@Param("farmerId") Long farmerId,
                                          @Param("startDateTime") LocalDateTime startDateTime,
                                          @Param("endDateTime") LocalDateTime endDateTime);

    /**
     * Query tính tổng doanh thu theo ngày cho farmer cụ thể (theo trạng thái cụ thể).
     * Trả về Tuple với cột 0 là LocalDate (orderDate), cột 1 là BigDecimal (dailyRevenue).
     */
    @Query("""
           ...
           ORDER BY orderDate ASC
           """)
    List<Tuple> sumRevenueByDayForFarmer(@Param("farmerId") Long farmerId,
                                         @Param("startDateTime") LocalDateTime startDateTime,
                                         @Param("endDateTime") LocalDateTime endDateTime,
