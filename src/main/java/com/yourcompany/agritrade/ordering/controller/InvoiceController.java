...
import com.yourcompany.agritrade.ordering.repository.OrderRepository; // Import OrderRepository
...
public class InvoiceController {
    ...
    public ResponseEntity<InputStreamResource> downloadInvoiceByOrderId(@PathVariable Long orderId) {
        ...
        headers.add(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=invoice-" + invoice.getInvoiceNumber() + ".pdf");
    ...
    public ResponseEntity<InputStreamResource> downloadInvoiceById(@PathVariable Long invoiceId) {
        ...
        Invoice invoice = invoiceRepository.findById(invoiceId) // Cần inject InvoiceRepository
                .orElseThrow(() -> new ResourceNotFoundException("Invoice", "id", invoiceId));

        ByteArrayInputStream pdfInputStream = invoiceService.generateInvoicePdf(invoiceId);
        HttpHeaders headers = new HttpHeaders();
        headers.add(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=invoice-" + invoice.getInvoiceNumber() + ".pdf");
        return ResponseEntity.ok().headers(headers).contentType(MediaType.APPLICATION_PDF).body(new InputStreamResource(pdfInputStream));
    }
}