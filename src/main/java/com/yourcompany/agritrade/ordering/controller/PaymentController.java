...
public class PaymentController {
    ...
    @Value("${payment.vnpay.hashSecret}") private String vnpHashSecret; // Inject hashSecret
    @Value("${payment.momo.secretKey}") private String momoSecretKey; // Inject MoMo Secret Key
    ...
    public ResponseEntity<Map<String, String>> handleVnpayIpn(@RequestParam Map<String, String> vnpayParams) {
        ...
        try {
            ...
            // 2. Tạo lại chuỗi hash data từ các tham số khác (loại bỏ vnp_SecureHash và vnp_SecureHashType)
            ...
            if (calculatedSecureHash.equals(receivedSecureHash)) {
                ...
                PaymentCallbackRequest callbackData = convertVnpayParamsToDto(vnpayParams); // Chuyển đổi sang DTO
                paymentService.handlePaymentCallback("VNPAY_IPN", callbackData); // Truyền thêm rawParams nếu cần

                // Phản hồi cho VNPay theo tài liệu
                response.put("RspCode", "00");
                response.put("Message", "Confirm Success");
            } else {
                log.error("VNPay IPN Error: Invalid Signature. Received: {}, Calculated: {}", receivedSecureHash, calculatedSecureHash);
    ...
    private PaymentCallbackRequest convertVnpayParamsToDto(Map<String, String> params) {
        ...
        try {
            if (params.containsKey("vnp_Amount")) {
                data.setAmount(new BigDecimal(params.get("vnp_Amount")).divide(new BigDecimal(100))); // Chia lại cho 100
            }
...
//    }
    ...
    public ResponseEntity<Void> handleMomoIpn(@RequestBody Map<String, Object> momoParams) {
        ...
        try {
            ...
            // 2. Tạo lại rawHashData từ các tham số khác (THEO ĐÚNG THỨ TỰ VÀ TÊN TRƯỜNG MO MO QUY ĐỊNH CHO IPN)
            ...
            String accessKey = (String) momoParams.get("accessKey"); // MoMo có thể không gửi lại accessKey trong IPN
            ...
            String rawHashData = "accessKey=" + accessKey + // Có thể MoMo không dùng accessKey ở đây
                    "&amount=" + amount +
                    "&extraData=" + extraData +
                    "&message=" + message +
                    "&orderId=" + orderId +
                    ...
                    "&transId=" + transId;
            // LƯU Ý: Đây chỉ là ví dụ, bạn PHẢI xem tài liệu MoMo để biết chính xác các trường và thứ tự cho IPN signature.
            ...
            if (calculatedSignature.equals(receivedSignature)) {
                ...
                log.error("MoMo IPN Error: Invalid Signature. Received: {}, Calculated: {}, RawData: {}",
                        receivedSignature, calculatedSignature, rawHashData);
    ...
    private PaymentCallbackRequest convertMomoParamsToDto(Map<String, Object> params) {
        PaymentCallbackRequest data = new PaymentCallbackRequest();
        data.setOrderCode((String) params.get("orderId")); // Đây là orderId của MoMo, bạn cần map nó về orderCode của bạn
        // Hoặc bạn đã lưu orderCode của bạn vào extraData khi tạo request
        data.setTransactionCode(String.valueOf(params.get("transId")));
        data.setSuccess(Integer.valueOf(0).equals(params.get("resultCode"))); // resultCode = 0 là thành công
        try {
