...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
  ...
  @Modifying
  @Query(
      "UPDATE Notification n SET n.isRead = true, n.readAt = :readAt WHERE n.recipient.id = :recipientId AND n.isRead = false")
  int markAllAsReadForRecipient(
      @Param("recipientId") Long recipientId, @Param("readAt") LocalDateTime readAt);

  // Đánh dấu một thông báo cụ thể là đã đọc
  @Modifying
  @Query(
      "UPDATE Notification n SET n.isRead = true, n.readAt = :readAt WHERE n.id = :notificationId AND n.recipient.id = :recipientId")
  int markAsRead(
