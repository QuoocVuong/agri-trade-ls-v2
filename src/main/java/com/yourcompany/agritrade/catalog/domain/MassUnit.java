...
public enum MassUnit {
    ...
    public BigDecimal convertToKg(BigDecimal quantity) {
        ...
        return quantity.multiply(this.kgFactor).setScale(3, RoundingMode.HALF_UP); // Giữ 3 chữ số thập phân cho kg
    }
    ...
    public BigDecimal convertFromKg(BigDecimal kgQuantity) {
        ...
        if (this.kgFactor.compareTo(BigDecimal.ZERO) == 0) {
            throw new ArithmeticException("Cannot convert from KG: kgFactor is zero for unit " + this.name());
        }
