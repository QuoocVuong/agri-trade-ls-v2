...
import com.yourcompany.agritradels.notification.mapper.NotificationMapper;
...
import com.yourcompany.agritradels.usermanagement.repository.UserRepository; // Import UserRepository
...
public class NotificationServiceImpl implements NotificationService {
    ...
    private final InAppNotificationService inAppNotificationService; // Inject InAppNotificationService
    ...
    public void sendOrderPlacementNotification(Order order) {
        // Gửi cho người mua
        String buyerMsg = String.format("Đơn hàng #%s của bạn đã được đặt thành công.", order.getOrderCode());
        String buyerLink = frontendUrl + "/orders/" + order.getId(); // Link đến chi tiết đơn hàng
        inAppNotificationService.createAndSendInAppNotification(order.getBuyer(), buyerMsg, NotificationType.ORDER_PLACED, buyerLink);
        // emailService.sendOrderConfirmationEmailToBuyer(order); // Gọi gửi mail

        // Gửi cho người bán
        String farmerMsg = String.format("Bạn có đơn hàng mới #%s từ %s.", order.getOrderCode(), order.getBuyer().getFullName());
        String farmerLink = frontendUrl + "/farmer/orders/" + order.getId();
        inAppNotificationService.createAndSendInAppNotification(order.getFarmer(), farmerMsg, NotificationType.ORDER_PLACED, farmerLink);
    ...
    public void sendOrderStatusUpdateNotification(Order order, OrderStatus previousStatus) {
        // Gửi cho người mua
        String buyerMsg = String.format("Đơn hàng #%s của bạn đã được cập nhật trạng thái thành: %s.", order.getOrderCode(), order.getStatus().name());
        String buyerLink = frontendUrl + "/orders/" + order.getId();
        inAppNotificationService.createAndSendInAppNotification(order.getBuyer(), buyerMsg, NotificationType.ORDER_STATUS_UPDATE, buyerLink);
        // emailService.sendOrderStatusUpdateEmailToBuyer(order, previousStatus);

        // Gửi cho người bán (nếu trạng thái thay đổi không phải do chính họ) - cần thêm logic kiểm tra người thực hiện
        if (order.getStatus() == OrderStatus.DELIVERED || order.getStatus() == OrderStatus.CANCELLED) { // Ví dụ chỉ báo cho farmer khi hoàn thành/hủy
            String farmerMsg = String.format("Đơn hàng #%s đã cập nhật trạng thái thành: %s.", order.getOrderCode(), order.getStatus().name());
            String farmerLink = frontendUrl + "/farmer/orders/" + order.getId();
            inAppNotificationService.createAndSendInAppNotification(order.getFarmer(), farmerMsg, NotificationType.ORDER_STATUS_UPDATE, farmerLink);
        }
    }
    ...
    public void sendOrderCancellationNotification(Order order) {
        ...
        String buyerLink = frontendUrl + "/orders/" + order.getId();
        inAppNotificationService.createAndSendInAppNotification(order.getBuyer(), buyerMsg, NotificationType.ORDER_CANCELLED, buyerLink);
        ...
        String farmerLink = frontendUrl + "/farmer/orders/" + order.getId();
        inAppNotificationService.createAndSendInAppNotification(order.getFarmer(), farmerMsg, NotificationType.ORDER_CANCELLED, farmerLink);
        // emailService.sendOrderCancellationNotificationToFarmer(order);
    }

    @Override
    public void sendPaymentSuccessNotification(Order order) {
        String buyerMsg = String.format("Thanh toán cho đơn hàng #%s đã thành công.", order.getOrderCode());
        String buyerLink = frontendUrl + "/orders/" + order.getId();
        inAppNotificationService.createAndSendInAppNotification(order.getBuyer(), buyerMsg, NotificationType.PAYMENT_SUCCESS, buyerLink);
    ...
    public void sendPaymentFailureNotification(Order order) {
        String buyerMsg = String.format("Thanh toán cho đơn hàng #%s đã thất bại.", order.getOrderCode());
        String buyerLink = frontendUrl + "/orders/" + order.getId();
    ...
    public void sendNewChatMessageNotification(User recipient, User sender, Long roomId) {
        ...
        inAppNotificationService.createAndSendInAppNotification(recipient, message, NotificationType.NEW_MESSAGE, link);
    ...
    public Page<NotificationResponse> getMyNotifications(Authentication authentication, Pageable pageable) {
        ...
        Page<Notification> notificationPage = notificationRepository.findByRecipientIdOrderByCreatedAtDesc(recipient.getId(), pageable);
    ...
    public void markNotificationAsRead(Authentication authentication, Long notificationId) {
        User recipient = getUserFromAuthentication(authentication);
        int updated = notificationRepository.markAsRead(notificationId, recipient.getId(), LocalDateTime.now());
        if (updated == 0) {
            // Có thể do notification không tồn tại hoặc không thuộc user
            log.warn("Failed to mark notification {} as read for user {}", notificationId, recipient.getId());
    ...
    public void markAllMyNotificationsAsRead(Authentication authentication) {
        User recipient = getUserFromAuthentication(authentication);
        int updatedCount = notificationRepository.markAllAsReadForRecipient(recipient.getId(), LocalDateTime.now());
    ...
    public void sendWelcomeNotification(User user) {
        ...
        inAppNotificationService.createAndSendInAppNotification(user, message, NotificationType.WELCOME, link);
    }
    ...
    public void sendPasswordChangedNotification(User user) {
        ...
        // inAppNotificationService.createAndSendInAppNotification(user, message, NotificationType.PASSWORD_RESET, null);
    }

    @Override
    public void sendAccountStatusUpdateNotification(User user, boolean newStatus) {
        log.info("Sending account status update notification to user {}. New status: {}", user.getId(), newStatus ? "ACTIVE" : "INACTIVE");
        ...
        inAppNotificationService.createAndSendInAppNotification(user, message, NotificationType.OTHER, link); // Có thể tạo Type riêng
    ...
    public void sendRolesUpdateNotification(User user) {
        ...
        String message = String.format("Vai trò của bạn trên hệ thống đã được cập nhật thành: %s.", rolesString);
    ...
    public void sendNewFollowerNotification(User followedUser, User follower) {
        ...
        String message = String.format("<strong>%s</strong> đã bắt đầu theo dõi bạn.", follower.getFullName());
        // Link đến trang profile của người follower
        String link = frontendUrl + "/profile/" + follower.getId(); // Giả sử có route này
        inAppNotificationService.createAndSendInAppNotification(followedUser, message, NotificationType.NEW_FOLLOWER, link);
    ...
    public void sendReviewApprovedNotification(Review review) {
        ...
        String message = String.format("Đánh giá của bạn cho sản phẩm '%s' đã được duyệt.", productName);
        // Link đến sản phẩm đã review
        String link = (review.getProduct() != null) ? frontendUrl + "/products/" + review.getProduct().getSlug() : null;
        inAppNotificationService.createAndSendInAppNotification(review.getConsumer(), message, NotificationType.REVIEW_APPROVED, link);
        // Có thể gửi cả email
    ...
    public void sendReviewRejectedNotification(Review review) {
        ...
        String productName = review.getProduct() != null ? review.getProduct().getName() : "sản phẩm";
        String message = String.format("Rất tiếc, đánh giá của bạn cho sản phẩm '%s' đã không được duyệt.", productName);
        // Có thể không cần link hoặc link đến trang quản lý review của user
        String link = frontendUrl + "/my-reviews"; // Ví dụ
        inAppNotificationService.createAndSendInAppNotification(review.getConsumer(), message, NotificationType.REVIEW_REJECTED, link);
        // Có thể gửi cả email
    ...
    public void sendFarmerProfileApprovedNotification(FarmerProfile profile) {
        ...
        // Gửi In-App notification
        String message = String.format("Chúc mừng! Hồ sơ nông dân '%s' của bạn đã được duyệt.", profile.getFarmName());
        String link = frontendUrl + "/farmer/dashboard"; // Link đến dashboard farmer
        inAppNotificationService.createAndSendInAppNotification(farmer, message, NotificationType.FARMER_PROFILE_APPROVED, link);

        // Gửi Email thông báo (tùy chọn)
        // emailService.sendFarmerProfileApprovedEmail(profile); // Cần tạo hàm này và template trong EmailService
    }
    ...
    public void sendFarmerProfileRejectedNotification(FarmerProfile profile, String reason) {
        ...
        User farmer = profile.getUser();
        log.info("Sending farmer profile rejected notification to user {}. Reason: {}", farmer.getId(), reason);

        // Gửi In-App notification
        String reasonText = StringUtils.hasText(reason) ? " Lý do: " + reason : "";
        String message = String.format("Rất tiếc, hồ sơ nông dân '%s' của bạn đã bị từ chối.%s", profile.getFarmName(), reasonText);
        String link = frontendUrl + "/user/profile/farmer-profile"; // Link đến trang sửa profile farmer
        inAppNotificationService.createAndSendInAppNotification(farmer, message, NotificationType.FARMER_PROFILE_REJECTED, link);

        // Gửi Email thông báo (tùy chọn)
        // emailService.sendFarmerProfileRejectedEmail(profile, reason); // Cần tạo hàm này và template trong EmailService
    }
