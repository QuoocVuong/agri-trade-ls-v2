package com.yourcompany.agritradels.interaction.websocket;
...
import com.yourcompany.agritradels.interaction.dto.event.WebSocketErrorEvent;
...
import org.springframework.messaging.handler.annotation.MessageMapping; // Import MessageMapping
import org.springframework.messaging.handler.annotation.Payload; // Import Payload
import org.springframework.messaging.simp.SimpMessageHeaderAccessor; // Import HeaderAccessor
// Import SimpMessagingTemplate nếu muốn gửi lại cho người gửi hoặc phòng chat
// import org.springframework.messaging.simp.SimpMessagingTemplate;
import org.springframework.messaging.simp.SimpMessagingTemplate;
import org.springframework.messaging.simp.annotation.SendToUser;
import org.springframework.security.access.AccessDeniedException;
import org.springframework.security.core.Authentication; // Lấy thông tin user từ Principal
...
//     * @param headerAccessor Chứa thông tin về session WebSocket và Principal (user đã xác thực).
//     */
//    @MessageMapping("/chat.sendMessage") // Endpoint nhận tin nhắn từ client
//    // @SendToUser("/queue/messages") // Không dùng SendToUser trực tiếp ở đây vì cần logic phức tạp hơn
...
//
//        log.info("Received chat message via WebSocket from {}: {}", authentication.getName(), messageRequest.getContent());
//
//        try {
//            // Gọi ChatService để lưu và gửi tin nhắn (Service sẽ tự push qua WebSocket đến người nhận)
//            ChatMessageResponse savedMessage = chatService.sendMessage(authentication, messageRequest);
//
...
//
//        } catch (Exception e) {
//            log.error("Error processing WebSocket chat message from {}: {}", authentication.getName(), e.getMessage(), e);
...
//            // messagingTemplate.convertAndSend(errorDestination, "Error sending message: " + e.getMessage());
//        }
//    }
    ...
    public void sendMessage(@Payload ChatMessageRequest messageRequest,
        ...
        log.info("WS Received: /chat.sendMessage from {}: {}", authentication.getName(), messageRequest.getContent());
...
//        }
//        Authentication authentication = (Authentication) principal;
//        log.info("Received mark read request via WebSocket from {} for room {}", authentication.getName(), roomId);
//        try {
//            chatService.markMessagesAsRead(authentication, roomId);
//            // TODO: Gửi thông báo đã đọc cho người kia trong phòng chat nếu cần
//        } catch (Exception e) {
//            log.error("Error processing mark read request from {}: {}", authentication.getName(), e.getMessage(), e);
//        }
//    }
    ...
    @MessageExceptionHandler({BadRequestException.class, ResourceNotFoundException.class, AccessDeniedException.class, Exception.class})
    ...
    public WebSocketErrorEvent handleException(Exception exception, Principal principal) {
        ...
        } else if (!(exception instanceof BadRequestException || exception instanceof ResourceNotFoundException)) {
    ...
    private Authentication getAuthentication(SimpMessageHeaderAccessor headerAccessor) {
        ...
        log.error("Cannot get Authentication from WebSocket session: {}", headerAccessor.getSessionId());
