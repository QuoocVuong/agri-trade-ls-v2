...
import com.yourcompany.agritradels.usermanagement.service.FarmerProfileService;
...
public class FarmerProfileServiceImpl implements FarmerProfileService {
    ...
    @Override
    @Transactional
    public FarmerProfileResponse createOrUpdateFarmerProfile(Authentication authentication, FarmerProfileRequest request) {
        User user = getUserFromAuthentication(authentication);

        // Kiểm tra tỉnh Lạng Sơn
//        if (!LANG_SON_PROVINCE_CODE.equals(request.getProvinceCode())) {
//            throw new BadRequestException("Farmer profile province must be Lang Son (" + LANG_SON_PROVINCE_CODE + ").");
//        }

        boolean isNewProfile = !farmerProfileRepository.existsById(user.getId()); // Kiểm tra xem có phải tạo mới không


        FarmerProfile profile = farmerProfileRepository.findById(user.getId())
                .orElseGet(() -> {
        ...
        if (profile.getUserId() != null) { // Nếu là cập nhật
            ...
            profile.setVerificationStatus(isNewProfile ? VerificationStatus.PENDING : profile.getVerificationStatus());
        } else { // Nếu là tạo mới
        ...
        if (isNewProfile) {
            Role farmerRole = roleRepository.findByName(RoleType.ROLE_FARMER)
                    .orElseThrow(() -> new ResourceNotFoundException("Role", "name", RoleType.ROLE_FARMER.name()));
